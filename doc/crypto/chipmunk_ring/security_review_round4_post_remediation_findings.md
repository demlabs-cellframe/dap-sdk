...
| CR-D35  | INFO       | `tests/unit/crypto/chipmunk_ring/`  | Coverage gaps: embedded-vs-non-embedded full wire roundtrip + concurrent sign on independent rings + opt-in max-k stress — addressed in this round.  Allocator-failure injection (the only remaining sub-item) requires a global mock-allocator and is tracked separately as `CR-INFRA-1`. | **CLOSED** (84ce28f4 + max-k commit) |
| CR-INFRA-1 | INFO    | infra (`tests/`)                    | Build a project-wide allocator-failure injection harness (LD_PRELOAD-based malloc/calloc shim + thread-local fail-on-Nth-call counter) so future leak/abort regressions in any module can be exercised deterministically.  Standalone infra task, not bound to ChipmunkRing. | OPEN (separate task) |
