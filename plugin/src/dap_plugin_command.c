...
static int s_command_handler(int a_argc, char **a_argv, void **a_str_reply, int a_version)
{
    ...
    switch (l_cmd_name) {
        ...
        case CMD_RESTART: {
            ...
            dap_plugin_stop_all();
            int l_total_errors = dap_plugin_start_all();
            if (l_total_errors) {
                log_it(L_WARNING, "Restart completed with %d errors", l_total_errors);
                dap_cli_server_cmd_set_reply_text(a_str_reply,
                                                  "Restart completed with %d errors. Check logs.", l_total_errors);
            } else {
