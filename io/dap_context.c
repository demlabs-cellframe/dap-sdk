...
int dap_context_init()
{
    ...
    if (setrlimit(RLIMIT_NOFILE, &l_fdlimit))
        return -2;
    log_it(L_INFO, "Set maximum opened descriptors from %" DAP_UINT64_FORMAT_U " to %"DAP_UINT64_FORMAT_U, (uint64_t)l_oldlimit, (uint64_t)l_fdlimit.rlim_cur);
#endif
    return 0;
}
...
#error "Unimplemented fetch esocket after poll"
            ...
            if (l_flag_read && !(l_cur->flags & DAP_SOCK_SIGNAL_CLOSE)) {
                ...
                // For new connections, new_callback may not have run yet if
                // dap_worker_add_events_socket queued it via eventfd.  Process
                // the callback synchronously here so _inheritor is set before
                // we read from the socket.
                if (l_cur->type == DESCRIPTOR_TYPE_SOCKET_CLIENT
                    && !l_cur->_inheritor
                    && !l_cur->is_initalized
                    && l_cur->callbacks.new_callback) {
                    l_cur->callbacks.new_callback(l_cur, NULL);
                    l_cur->is_initalized = true;
                }
                ...
                if (l_must_read_smth){ // Socket/Descriptor read
                    if(l_bytes_read > 0) {
                        ...
                        if (l_cur->callbacks.read_callback) {
                            ...
                            /* TUN read may free kernel TX queue — retry pending egress. */
                            if (l_cur->type == DESCRIPTOR_TYPE_FILE && l_cur->buf_out_size > 0)
                    ...
                    case DESCRIPTOR_TYPE_FILE:
                        ...
                        if (l_bytes_sent < 0 && (l_errno == EAGAIN || l_errno == EWOULDBLOCK)) {
                            if (l_cur->type == DESCRIPTOR_TYPE_FILE) {
                                ...
                                debug_if(s_debug_more, L_DEBUG,
                                         "PIPE fd=%d write EAGAIN — dropping %zu bytes to avoid busy loop",
                                         s_es_io_fd(l_cur), l_cur->buf_out_size);
                                l_cur->buf_out_size = 0;
                                dap_events_socket_set_writable_unsafe(l_cur, false);
                                l_bytes_sent = 0;
