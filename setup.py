...
def build_and_copy_native_library():
    ...
    print("   Please run 'cmake --build build_new' first to compile the extension")
    return False
...
setup(
    ...
    py_modules=['python_dap'],  # Add python_dap as top-level module
    package_data={
        "": ["python_dap.so"],  # Include top-level python_dap.so
        'dap': ['py.typed', '*.so', 'python_dap.so'],  # Include .so files in dap package
        '': ['lib/*.so']  # Also include lib/*.so for backwards compatibility
    },
