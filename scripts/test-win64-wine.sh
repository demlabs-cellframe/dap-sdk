#!/bin/bash
# Run Windows x64 tests using Wine
# Usage: ./scripts/test-win64-wine.sh [test_pattern]
#
# Prerequisites:
#   1. Run ./scripts/deb-test-win64-setup.sh (as root)
#   2. Run ./scripts/test-win64-cross.sh to build
#   3. Run this script to execute tests
#
# Examples:
#   ./scripts/test-win64-wine.sh              # Run all tests
#   ./scripts/test-win64-wine.sh test_unit    # Run tests matching "test_unit"
#   ./scripts/test-win64-wine.sh test_json    # Run JSON tests only

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
BUILD_DIR="$PROJECT_ROOT/build-cross-win64"
...
check_prerequisites() {
    if ! command -v wine64 &> /dev/null; then
        log_error "wine64 not found. Run: sudo ./scripts/deb-test-win64-setup.sh"
        exit 1
    fi
    
    if [ ! -d "$BUILD_DIR" ]; then
        log_error "Build directory not found: $BUILD_DIR"
        log_error "Run: ./scripts/test-win64-cross.sh"
        exit 1
    fi
}
...
setup_wine_env() {
    ...
    # Use separate prefix for testing
    export WINEPREFIX="${HOME}/.wine-dap-test-64"
...
run_tests() {
    ...
    for test_exe in "${test_files[@]}"; do
        ...
        if timeout 60 wine64 "$test_exe" 2>&1; then
...
main() {
    echo "=============================================="
    echo "DAP SDK Windows x64 Tests (Wine)"
    echo "=============================================="
    echo ""
    ...
    log_info "Wine version: $(wine64 --version)"
