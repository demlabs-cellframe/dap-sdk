...
#define CHUNK_SIZE_VALUE ((size_t)32)
...
#define SIMD_LOAD(ptr)           _mm256_loadu_si256((const VECTOR_TYPE *)(ptr))
...
static size_t s_scan_string_fast_avx2(
...
)
{
    ...
    while (l_pos + chunk_size <= l_len) {
        ...
        MASK_TYPE quote_mask = (MASK_TYPE)_mm256_movemask_epi8(has_quote);
        MASK_TYPE backslash_mask = (MASK_TYPE)_mm256_movemask_epi8(has_backslash);
...
int dap_json_stage1_run_avx2(dap_json_stage1_t *a_stage1)
{
    ...
    debug_if(dap_json_get_debug(), L_DEBUG, "Starting AVX2 SimdJSON Stage 1 tokenization (%zu bytes)", input_len);
    ...
    while (pos < chunk_end) {
        ...
        while (chunk_pos < chunk_limit) {
            ...
            if (c == '"') {
                size_t end = s_scan_string_fast_avx2(a_stage1, chunk_pos);
    ...
    while (pos < input_len) {
        ...
        // String
            ...
            size_t end = s_scan_string_fast_avx2(a_stage1, pos);
    ...
    debug_if(dap_json_get_debug(), L_DEBUG, "AVX2 SimdJSON Stage 1 complete: %zu tokens (%zu structural, %zu strings, %zu numbers, %zu literals)",
