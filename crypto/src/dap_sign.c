...
static void s_sign_hash_to_str(char *a_str, size_t a_str_size, dap_chain_hash_fast_t *a_hash, const char *a_hash_out_type)
{
    if (!a_str_size)
        return;
    if (dap_strcmp(a_hash_out_type, "hex")) {
        dap_enc_b58_hash_str_t l_hash_str = dap_enc_base58_encode_hash_to_str_static_(a_hash);
        strncpy(a_str, l_hash_str.s, a_str_size);
        a_str[a_str_size - 1] = '\0';
        return;
    }
    dap_chain_hash_fast_to_str(a_hash, a_str, a_str_size);
}
...
void dap_sign_get_information(dap_sign_t* a_sign, dap_string_t *a_str_out, const char *a_hash_out_type)
{
    ...
    if(dap_sign_get_pkey_hash(a_sign, &l_hash_pkey)) {
        ...
        s_sign_hash_to_str(l_hash_str, sizeof(l_hash_str), &l_hash_pkey, a_hash_out_type);
...
void dap_sign_get_information_json(json_object* a_json_arr_reply, dap_sign_t* a_sign, json_object *a_json_out, const char *a_hash_out_type, int a_version)
{
    ...
    if(dap_sign_get_pkey_hash(a_sign, &l_hash_pkey)) {
        char l_hash_str[DAP_CHAIN_HASH_FAST_STR_SIZE];
        s_sign_hash_to_str(l_hash_str, sizeof(l_hash_str), &l_hash_pkey, a_hash_out_type);
        json_object_object_add(a_json_out, a_version == 1 ? "Public key hash" : "pkey_hash",
