...
static void test_signing_verifying(int a_times, int *a_sig_time, int *a_verify_time)
{
    ...
    dap_enc_key_t *l_key = dap_enc_key_new_generate(DAP_ENC_KEY_TYPE_SIG_TESLA, NULL, 0, seed, seed_size, 0);
    ...
    for (int i = 0; i < a_times; ++i) {
        ...
        l_keys[i] = dap_enc_key_new_generate(DAP_ENC_KEY_TYPE_SIG_TESLA, NULL, 0, seed, seed_size, 0);
    ...
    *a_sig_time = l_t2 - l_t1;
    ...
    for(int i = 0; i < a_times; ++i) {
        ...
        tesla_signature_delete((tesla_signature_t*)l_signs[i]);
...
void dap_enc_tesla_tests_run(int a_times)
{
    dap_print_module_name("TESLA");
    init_test_case();
