...
static void test_signing_verifying(int a_times, int *a_sig_time, int *a_verify_time)
{
    ...
    dap_enc_key_t *l_key = dap_enc_key_new_generate(DAP_ENC_KEY_TYPE_SIG_BLISS, NULL, 0, seed, seed_size, 0);
    ...
    for (int i = 0; i < a_times; ++i) {
        ...
        l_keys[i] = dap_enc_key_new_generate(DAP_ENC_KEY_TYPE_SIG_BLISS, NULL, 0, seed, seed_size, 0);
    ...
    *a_sig_time = l_t2 - l_t1;
    ...
    for(int i = 0; i < a_times; ++i) {
        ...
        bliss_signature_delete((bliss_signature_t*)l_signs[i]);
...
static void test_signing_verifying_serial(int a_times, int *a_sig_time, int *a_verify_time)
{
    ...
    for (int i = 0; i < a_times; ++i) {
        ...
        dap_enc_key_t *key = dap_enc_key_new_generate(DAP_ENC_KEY_TYPE_SIG_BLISS, NULL, 0, seed, seed_size, 0);
...
void dap_enc_bliss_tests_run(int a_times)
{
    dap_print_module_name("BLISS");
    init_test_case();
