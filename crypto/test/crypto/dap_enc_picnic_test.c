...
static void test_signing_verifying(int a_times, int *a_sig_time, int *a_verify_time)
{
    ...
    dap_enc_key_t *l_key = dap_enc_key_new_generate(DAP_ENC_KEY_TYPE_SIG_PICNIC, NULL, 0, seed, seed_size, 0);
    ...
    for (int i = 0; i < a_times; ++i) {
        ...
        l_keys[i] = dap_enc_key_new_generate(DAP_ENC_KEY_TYPE_SIG_PICNIC, NULL, 0, seed, seed_size, 0);
...
void dap_enc_picnic_tests_run(int a_times)
{
    dap_print_module_name("PICNIC");
    init_test_case();
