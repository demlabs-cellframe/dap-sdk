...
\textbf{Definition 1 (Ring-LWE Problem):} Let $R = \mathbb{Z}[X]/(X^n + 1)$ be the ring of integers modulo the $n$-th cyclotomic polynomial, and $R_q = R/qR$. The Ring-LWE problem asks to distinguish between samples $(a_i, b_i)$ where either:
\begin{itemize}
\item $(a_i, b_i)$ are uniformly random in $R_q \times R_q$, or
\item $b_i = a_i \cdot s + e_i$ for fixed secret $s \in R_q$ and error terms $e_i$ sampled from a Gaussian distribution
\end{itemize}
...
\item \textbf{Performance}: 100-1000× faster than existing post-quantum implementations
\item \textbf{Scalability}: Linear signature size growth with ring size (vs quadratic in some schemes)
\item \textbf{Practicality}: First post-quantum ring signature with sub-millisecond performance
\item \textbf{Blockchain Ready}: Signature sizes and timing suitable for real-world blockchain deployment
\end{itemize}
