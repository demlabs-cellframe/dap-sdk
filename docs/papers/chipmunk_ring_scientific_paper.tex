...
\COMMENT{$p$ and $I$ are taken from signature parameters: single-signer uses $p=64$ bytes and $I=100$ by default; multi-signer uses $p=96$ bytes and $I\in\{1000,10000\}$}
\ENDFOR
\STATE Serialize $D \leftarrow \text{Serialize}(M, \text{ring\_hash}, \{\text{acorn\_proofs}\})$
\STATE Compute challenge $c = \text{SHA3-256}(D)$ \COMMENT{32-byte SHA3-256 hash of serialized data}
...
\STATE Serialize $D' \leftarrow \text{Serialize}(M, \text{ring\_hash}, \{\text{acorn\_proofs}\})$ and compute $c' = \text{SHA3-256}(D')$
\IF{$c' \neq c$} \RETURN Reject \ENDIF
\IF{$t = 1$} \COMMENT{Traditional ring (OR) semantics}
...
\IF{$\text{expected}_i = \text{acorn\_proof}_i$} $valid \leftarrow valid + 1$ \ENDIF
\ENDFOR
\STATE Accept iff $valid \ge 1$
\ELSE \COMMENT{Multi-signer (threshold) mode}
