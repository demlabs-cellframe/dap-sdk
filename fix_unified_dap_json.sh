...
# Script to fix dap_json.c to use unified opaque types properly

DAP_JSON_C="module/core/src/dap_json.c"

echo "Fixing dap_json.c to use unified opaque types..."

# Backup original file
cp "$DAP_JSON_C" "$DAP_JSON_C.backup2"

# 1. Fix return statements that return raw json_object* to use conversion helpers
sed -i 's/return json_object_array_get_idx(\([^,]*\), \([^)]*\))/return _json_c_to_dap_json(json_object_array_get_idx(_dap_json_to_json_c(\1), \2))/g' "$DAP_JSON_C"
...
sed -i 's/json_object_array_add(\([^,]*\), \([^)]*\))/json_object_array_add(_dap_json_to_json_c(\1), _dap_json_to_json_c(\2))/g' "$DAP_JSON_C"
sed -i 's/json_object_array_length(\([^)]*\))/json_object_array_length(_dap_json_to_json_c(\1))/g' "$DAP_JSON_C"
...
# Fix object adding with object parameters - need special handling
...
sed -i 's/json_object_object_add(_dap_json_to_json_c(\([^)]*\)), \([^,]*\), \(a_array\))/json_object_object_add(_dap_json_to_json_c(\1), \2, _dap_json_to_json_c(\3))/g' "$DAP_JSON_C"
