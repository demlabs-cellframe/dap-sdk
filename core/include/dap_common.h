...
#define dap_is_signed(v) ( dap_minval(v) < 0 )
    ...
    ({                                                          \
        ...
        if (_a_umax != dap_maxlluval && _b_umax != dap_maxlluval) { \
            ...
            { (_a += _b); } \
        } else if (_a_umax == dap_maxlluval) { \
            if (!( \
                (_b < 0 && _a < (typeof(_a))(dap_minval(_a) - _b)) || \
                (_b > 0 && _a > (typeof(_a))(dap_maxval(_a) - _b)) \
                )) \
            { (_a += _b); } \
        } else { \
            if (!( \
                (_b < 0 &&  _b < -((long long)(dap_maxuval(_a)))) || \
                (_b > 0 &&  (unsigned long long)_b > (dap_maxuval(_a)) ) || \
