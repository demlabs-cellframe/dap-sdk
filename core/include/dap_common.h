...
#define DAP_CALLOC(p, s)      ({ size_t s1 = (size_t)(s); s1 > 0 ? calloc(p, s1) : DAP_NULL_PTR(void); })
#define DAP_ALMALLOC(p, s)    ({ size_t s1 = (size_t)(s); s1 > 0 ? _dap_aligned_alloc(p, s1) : DAP_NULL_PTR(void); })
#define DAP_ALREALLOC(p, s)   ({ size_t s1 = (size_t)(s); s1 > 0 ? _dap_aligned_realloc(p, s1) :  DAP_NULL_PTR(void); })
...
#define DAP_NEW_SIZE(t, s)    ({ size_t s1 = (size_t)(s); s1 > 0 ? DAP_CAST_PTR(t, malloc(s1)) : DAP_NULL_PTR(t); })
 /* Auto memory! Do not inline! */
#define DAP_NEW_STACK(t)            DAP_CAST_PTR(t, alloca(sizeof(t)))
#define DAP_NEW_STACK_SIZE(t, s)    ({ size_t s1 = (size_t)(s); s1 > 0 ? DAP_CAST_PTR(t, alloca(s1)) : DAP_NULL_PTR(t); })
/* ... */
#define DAP_NEW_Z(t)          DAP_CAST_PTR(t, calloc(1, sizeof(t)))
#define DAP_NEW_Z_SIZE(t, s)  ({ size_t s1 = (size_t)(s); s1 > 0 ? DAP_CAST_PTR(t, calloc(1, s1)) : DAP_NULL_PTR(t); })
#define DAP_REALLOC(t, s)     ({ size_t s1 = (size_t)(s); s1 > 0 ? realloc(t, s1) : ({ DAP_DEL_Z(t); DAP_NULL_PTR(t); }); })
#define DAP_DELETE(p)         free((void*)(p))
#define DAP_DUP(p)            ({ void *p1 = (uintptr_t)(p) != 0 ? malloc(sizeof(*(p))) : NULL; p1 ? memcpy(p1, (p), sizeof(*(p))) : DAP_NULL_PTR(void); })
#define DAP_DUP_SIZE(p, s)    ({ void *p1 = (p) && (size_t)(s) > 0 ? malloc(s) : NULL; p1 ? memcpy(p1, (p), (s)) : DAP_NULL_PTR(void); })
#endif
