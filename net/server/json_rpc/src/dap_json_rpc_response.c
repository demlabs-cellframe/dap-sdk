...
dap_json_rpc_response_t *dap_json_rpc_response_creation(const char *result, dap_json_rpc_response_type_result_t type, int64_t id)
{
...
{
    if (response) {
        DAP_DEL_Z(response->result);
        DAP_DEL_Z(response->error);
        DAP_FREE(response);
    }
}
...
//                 l_obj_result = json_object_new_string(a_response->result);
//                 break;
//             case TYPE_RESPONSE_DOUBLE:
//                 l_obj_result = json_object_new_double(a_response->result);
//                 break;
//             case TYPE_RESPONSE_BOOLEAN:
//                 l_obj_result = json_object_new_boolean(a_response->result);
//                 break;
//             case TYPE_RESPONSE_INTEGER:
//                 l_obj_result = json_object_new_int64(a_response->result);
//                 break;
//             case TYPE_RESPONSE_JSON:
//                 l_obj_result = json_object_get(a_response->result);
//                 json_object_put(a_response->result);
//                 break;
//             default:{}
//         }
//     }else{
