...
//        }
        ...
        if ( marker == 2 ) {
            ...
            if ( argc >= 1) {
                ...
                if ( l_cmd ) {
                    bool l_cmd_logging = false;
                    while( list ) {
                                                l_cmd_logging |= !strcmp((char *)list->data, "-password") || !strcmp((char *)list->data, "password");
                        str_cmd = dap_strdup_printf( "%s;%s", str_cmd, list->data );
                        list = dap_list_next(list);
                    }

                    log_it(L_INFO, "execute command=%s", l_cmd_logging ? "Command hide, password used" : str_cmd );
                    // exec command
              ...
              int l_verbose = 0;
                ...
                if(l_cmd){
                    ...
                    if(l_cmd->overrides.log_cmd_call)
                        l_cmd->overrides.log_cmd_call(l_cmd_logging ? "Command hide, password used" : str_cmd);
                    else
                        log_it(L_DEBUG, "execute command=%s", l_cmd_logging ? "Command hide, password used" : str_cmd);
                    // exec command
