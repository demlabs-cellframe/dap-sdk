...
/**
 * @file dap_net_trans_http_stream.h
 * @brief HTTP Trans Adapter for DAP Stream
 * 
 * This module provides backward compatibility by wrapping the existing HTTP
 * implementation into the new Trans Abstraction Layer.
 * 
 * **Purpose:**
 * - Maintain 100% compatibility with legacy HTTP-based clients
 * - Bridge between old HTTP code and new trans architecture
 * - Translate HTTP query parameters to/from TLV handshake protocol
 * - Allow gradual migration from HTTP to alternative transs
 * 
 * **Architecture:**
 * ```
 * Legacy HTTP Code
 *        ↓
 * HTTP Trans Adapter ← You are here
 *        ↓
 ...
 * @date 2025-10-22
 * @version 1.0.0
 * @ingroup dap_stream_trans
 */

#ifndef DAP_STREAM_TRANS_HTTP_H
#define DAP_STREAM_TRANS_HTTP_H
...
/**
 * @defgroup dap_stream_trans_http HTTP Trans Adapter
 * @ingroup dap_stream_trans
 * @brief HTTP trans adapter for backward compatibility
 * @{
 */

/**
 * @brief HTTP trans private data structure
 * 
 * Contains HTTP-specific connection state and references
 * to legacy HTTP infrastructure.
 */
typedef struct dap_stream_trans_http_private {
    ...
    void *legacy_ctx;                   ///< Pointer to legacy HTTP ctx
} dap_stream_trans_http_private_t;

/**
 * @brief HTTP trans configuration
 */
typedef struct dap_stream_trans_http_config {
...
} dap_stream_trans_http_config_t;
 ...
 * 
 * Registers the HTTP trans implementation with the trans registry.
 ...
 * if (dap_stream_trans_http_register() < 0) {
 *     log_it(L_CRITICAL, "Failed to register HTTP trans");
 *     return -1;
 * }
 * @endcode
 */
 ...
 * 
 * Removes the HTTP trans from the trans registry.
 ...
 * but routes them through the new trans layer.
 * 
 ...
 * dap_stream_trans_http_add_proc(server, "/stream");
 * @endcode
 */
...
/**
 * @brief Get HTTP trans instance from stream
 * 
 * @param[in] a_stream Stream instance
 * 
 * @return HTTP trans private data, or NULL if not HTTP trans
 */
 ...
 * @param[in] a_stream Stream instance
 * 
 * @return true if HTTP trans, false otherwise
 */
 ...
 * 
 * This function sends an unencrypted HTTP request through the HTTP trans.
 ...
 * 
 * This function sends an encrypted HTTP request through the HTTP trans.
