...
void setcfg(dap_config_t* a_cfg) {
    __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "Set config %p", a_cfg);
    cli_config = a_cfg;
}
...
int testCall() {
    dap_cli_cmd_t *l1 = NULL, *l2 = NULL;
    setcfg(g_config);
    __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "Config %p", cli_config);
    HASH_ITER(hh, dap_cli_server_cmd_get_first(), l1, l2) {
         __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "Command %s", l1->name);
    }
    log_it(L_DEBUG, " CFG %p", g_config);
    return 0;
}
