...
void dap_cluster_delete(dap_cluster_t *a_cluster)
{
    if (!a_cluster)
        return;
    pthread_rwlock_wrlock(&s_clusters_rwlock);
    /* Virtual clusters are never added to s_clusters hashtable (dap_cluster_new
     * returns early for DAP_CLUSTER_TYPE_VIRTUAL). Guard HASH_DEL to avoid
     * crashing on clusters with hh.tbl == NULL. */
    if (a_cluster->hh.tbl)
        HASH_DEL(s_clusters, a_cluster);
    if (a_cluster->mnemonim) {
        if (a_cluster->hh_str.tbl)
...
json_object *dap_cluster_get_links_info_json(dap_cluster_t *a_cluster)
{
    ...
    if (l_links_info) {
        for (size_t i = 0; i < l_total_links_count; i++) {
            ...
            json_object *l_jobj_ip = json_object_new_string(l_link_info->remote_addr_str ? l_link_info->remote_addr_str : "");
            json_object *l_jobj_port = json_object_new_int(l_link_info->remote_port);
            json_object *l_jobj_channel = json_object_new_string(l_link_info->channels ? l_link_info->channels : "");
