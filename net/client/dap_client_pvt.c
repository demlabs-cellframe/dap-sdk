...
void dap_client_pvt_delete_unsafe(dap_client_pvt_t * a_client_pvt)
{
    ...
    DAP_DEL_Z(a_client_pvt->uplink_addr);
    DAP_DEL_Z(a_client_pvt->session_key_id);
    DAP_DEL_Z(a_client_pvt->active_channels);
...
static void s_stream_connected(dap_client_pvt_t * a_client_pvt)
{
    ...
    log_it(L_INFO, "[cl_pvt:%p] Remote address connected for streaming on (%s:%u) with Socket #%"DAP_FORMAT_SOCKET" (assign on worker #%u)", a_client_pvt,
