 ...
 * All rights reserved.
 *
 * dap_client_trans_ctx - Client IO/transport layer (minimal trans_ctx._inheritor)
 *
...
dap_client_trans_ctx_t *dap_client_trans_ctx_find(uint64_t a_uuid)
{
    ...
    HASH_FIND(hh, s_trans_ctx_table, &a_uuid, sizeof(uint64_t), l_result);
...
void dap_client_trans_ctx_register(dap_client_trans_ctx_t *a_ctx)
{
    ...
    HASH_FIND(hh, s_trans_ctx_table, &a_ctx->uuid, sizeof(uint64_t), l_existing);
    if (!l_existing)
        HASH_ADD(hh, s_trans_ctx_table, uuid, sizeof(uint64_t), a_ctx);
    pthread_rwlock_unlock(&s_trans_ctx_table_lock);
}
...
void dap_client_trans_ctx_unregister(dap_client_trans_ctx_t *a_ctx)
{
    ...
    HASH_FIND(hh, s_trans_ctx_table, &a_ctx->uuid, sizeof(uint64_t), l_existing);
    if (l_existing)
        HASH_DEL(s_trans_ctx_table, l_existing);
    pthread_rwlock_unlock(&s_trans_ctx_table_lock);
}
...
void dap_client_trans_ctx_deinit(void)
{
    ...
    HASH_ITER(hh, s_trans_ctx_table, l_current, l_tmp) {
        HASH_DEL(s_trans_ctx_table, l_current);
    }
    pthread_rwlock_unlock(&s_trans_ctx_table_lock);
}
...
/**
 * @brief Clean trans_ctx IO/crypto (on worker thread)
 */
...
static void s_enc_init_response(dap_client_t *a_client, const void *a_data, size_t a_data_size)
{
    ...
    dap_client_trans_ctx_t *l_ctx = l_fsm ? l_fsm->client_trans_ctx : NULL;
    dap_net_trans_ctx_t *l_tc = l_fsm ? l_fsm->trans_ctx : NULL;
    dap_return_if_pass(!l_ctx || !l_fsm || !l_tc || !a_data);
            ...
            json_object_object_foreach(jobj, key, val) {
                ...
                if (json_object_get_type(val) == json_type_int) {
                    ...
                    if (!strcmp(key, "dap_protocol_version")) {
                        l_tc->remote_protocol_version = val_int;
                        l_json_parse_count++;
                    }
                }
            }
...
static void s_stream_es_callback_delete(dap_events_socket_t *a_es, UNUSED_ARG void *a_arg)
{
    ...
    if (l_tc && l_tc->stream)
    {
        log_it(L_WARNING, "Stream state at delete: session=%p, channel_count=%zu, stream_size=%zu",
               l_tc->stream->session,
               l_tc->stream->channel_count,
               l_tc->stream->stream_size);
    }
...
static void s_stream_es_callback_read(dap_events_socket_t *a_es, void *arg)
{
    ...
    dap_client_stage_t l_stage = (dap_client_stage_t)atomic_load(&l_fsm->stage_readable);
...
static bool s_stream_es_callback_write(dap_events_socket_t *a_es, UNUSED_ARG void *a_arg)
{
    ...
    dap_client_trans_ctx_t *l_ctx = l_fsm ? l_fsm->client_trans_ctx : NULL;
    dap_net_trans_ctx_t *l_tc = l_fsm ? l_fsm->trans_ctx : NULL;
    if (!l_ctx || !l_fsm || !l_tc || !l_tc->stream) return false;

    l_ctx->ts_last_active = dap_time_now();

    dap_client_stage_t l_stage = (dap_client_stage_t)atomic_load(&l_fsm->stage_readable);
    dap_client_stage_status_t l_status = (dap_client_stage_status_t)atomic_load(&l_fsm->stage_status_readable);
    ...
    if (l_stage == STAGE_STREAM_STREAMING) {
        for (size_t i = 0; i < l_tc->stream->channel_count; i++) {
            dap_stream_ch_t *ch = l_tc->stream->channel[i];
