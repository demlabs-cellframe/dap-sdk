...
#define LOG_TAG "dap_net_transport_udp_server"

// Transport server operations callbacks
static void* s_udp_server_new(const char *a_server_name)
{
    return (void*)dap_net_transport_udp_server_new(a_server_name);
}

static int s_udp_server_start(void *a_server, const char *a_cfg_section, 
                               const char **a_addrs, uint16_t *a_ports, size_t a_count)
{
    dap_net_transport_udp_server_t *l_udp = (dap_net_transport_udp_server_t *)a_server;
    return dap_net_transport_udp_server_start(l_udp, a_cfg_section, a_addrs, a_ports, a_count);
}

static void s_udp_server_stop(void *a_server)
{
    dap_net_transport_udp_server_t *l_udp = (dap_net_transport_udp_server_t *)a_server;
    dap_net_transport_udp_server_stop(l_udp);
}

static void s_udp_server_delete(void *a_server)
{
    dap_net_transport_udp_server_t *l_udp = (dap_net_transport_udp_server_t *)a_server;
    dap_net_transport_udp_server_delete(l_udp);
}

static const dap_net_transport_server_ops_t s_udp_server_ops = {
...
dap_net_transport_udp_server_t *dap_net_transport_udp_server_new(const char *a_server_name)
{
    ...
    dap_net_transport_udp_server_t *l_udp_server = DAP_NEW_Z(dap_net_transport_udp_server_t);
    if (!l_udp_server) {
                                       ...
                                       size_t a_count)
{
    ...
    for (size_t i = 0; i < a_count; i++) {
        ...
        if (l_ret != 0) {
            ...
            dap_net_transport_udp_server_stop(a_udp_server);
            return -4;
        }
