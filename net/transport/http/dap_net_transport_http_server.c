...
{
    return (void*)dap_net_transport_http_server_new(a_server_name);
}

static int s_http_server_start(void *a_server, const char *a_cfg_section, 
                                const char **a_addrs, uint16_t *a_ports, size_t a_count)
{
    dap_net_transport_http_server_t *l_http = (dap_net_transport_http_server_t *)a_server;
    return dap_net_transport_http_server_start(l_http, a_cfg_section, a_addrs, a_ports, a_count);
}

static void s_http_server_stop(void *a_server)
{
    dap_net_transport_http_server_t *l_http = (dap_net_transport_http_server_t *)a_server;
    dap_net_transport_http_server_stop(l_http);
}

static void s_http_server_delete(void *a_server)
{
    dap_net_transport_http_server_t *l_http = (dap_net_transport_http_server_t *)a_server;
    dap_net_transport_http_server_delete(l_http);
}

static const dap_net_transport_server_ops_t s_http_server_ops = {
 ...
 */
int dap_net_transport_http_server_init(void)
{
    log_it(L_DEBUG, "dap_net_transport_http_server_init: Starting HTTP server initialization");
    ...
    log_it(L_DEBUG, "dap_net_transport_http_server_init: HTTP module initialized, initializing encryption");
 ...
 */
dap_net_transport_http_server_t *dap_net_transport_http_server_new(const char *a_server_name)
{
    if (!a_server_name) {
    ...
    dap_net_transport_http_server_t *l_http_server = DAP_NEW_Z(dap_net_transport_http_server_t);
    if (!l_http_server) {
 ...
 */
int dap_net_transport_http_server_start(dap_net_transport_http_server_t *a_http_server,
                                        ...
                                        size_t a_count)
{
    ...
    // Delete transport context (handlers are already registered)
    dap_net_transport_server_context_delete(l_context);
 ...
 */
void dap_net_transport_http_server_stop(dap_net_transport_http_server_t *a_http_server)
{
    if (!a_http_server) {
        return;
    }
 ...
 */
void dap_net_transport_http_server_delete(dap_net_transport_http_server_t *a_http_server)
{
    if (!a_http_server) {
        return;
    }
