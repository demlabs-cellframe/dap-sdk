...
# On APPLE: pick x86 and/or arm families from CMAKE_OSX_ARCHITECTURES when set,
# else from CMAKE_SYSTEM_PROCESSOR. (Do not merge x86 asm into a native arm64
# build — integrated assembler is arch-specific; .c multiversion attrs are not.)
# ============================================================================
function(dap_arch_add_sources)
    ...
    if(APPLE)
        set(_apple_want_x86 FALSE)
        set(_apple_want_arm FALSE)
        if(CMAKE_OSX_ARCHITECTURES)
            foreach(_a IN LISTS CMAKE_OSX_ARCHITECTURES)
                string(TOLOWER "${_a}" _al)
                if(_al STREQUAL "x86_64" OR _al STREQUAL "i386")
                    set(_apple_want_x86 TRUE)
                elseif(_al STREQUAL "arm64")
                    set(_apple_want_arm TRUE)
                endif()
            endforeach()
        else()
