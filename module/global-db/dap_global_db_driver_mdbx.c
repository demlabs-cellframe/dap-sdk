...
static void *s_db_mdbx_read_cond(const char *a_group, dap_global_db_driver_hash_t a_hash_from, size_t *a_count_out, bool a_keys_only_read, bool a_with_holes, bool a_prev)
{
    ...
    if (a_keys_only_read) {
        dap_global_db_hash_pkt_hdr_mem_t l_hhdr = { .hashes_count = 0, .group_name_len = (uint16_t)l_group_name_len };
        if (dap_global_db_hash_pkt_hdr_pack(&l_hhdr, (uint8_t *)l_obj_arr, DAP_GLOBAL_DB_HASH_PKT_HDR_WIRE_SIZE) != 0) {
            log_it(L_ERROR, "GLOBAL_DB hash packet header pack failed");
            DAP_DEL_Z(l_obj_arr);
            goto safe_ret;
        }
        memcpy((uint8_t *)l_obj_arr + DAP_GLOBAL_DB_HASH_PKT_HDR_WIRE_SIZE, a_group, l_group_name_len);
    }
...
safe_ret:
    ...
    if (a_keys_only_read && l_obj_arr) {
        dap_global_db_hash_pkt_hdr_mem_t l_hhdr = {
            .hashes_count = (uint32_t)l_count_current,
            .group_name_len = (uint16_t)l_group_name_len,
        };
        if (dap_global_db_hash_pkt_hdr_pack(&l_hhdr, (uint8_t *)l_obj_arr, DAP_GLOBAL_DB_HASH_PKT_HDR_WIRE_SIZE) != 0)
            log_it(L_ERROR, "GLOBAL_DB hash packet header final pack failed");
    }
    return l_obj_arr;
}
