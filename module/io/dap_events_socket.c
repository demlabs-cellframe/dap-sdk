...
const char *s_socket_type_to_str[DESCRIPTOR_TYPE_MAX] = { 
    "CLIENT", "LOCAL CLIENT", "SERVER", "LOCAL SERVER", "UDP CLIENT", "SSL CLIENT",
    "RAW", "FILE", "PIPE", "QUEUE", "TIMER", "EVENT"
};
...
typedef struct dap_queue_buf_thread_state {
    ...
    struct dap_queue_buf_thread_state *next;
} dap_queue_buf_thread_state_t;

static pthread_mutex_t s_queue_buf_threads_lock = PTHREAD_MUTEX_INITIALIZER;
static dap_queue_buf_thread_state_t *s_queue_buf_threads = NULL;
#endif

#ifndef DAP_EVENTS_CAPS_IOCP
static void s_queue_ops_enter(void);
static void s_queue_ops_leave(void);
static void s_queue_ops_wait_drain(void);
#endif
...
#endif
    debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Created blank es %p, uuid " DAP_FORMAT_ESOCKET_UUID,
                              l_es, l_es->uuid);
    return  l_es;
}
...
static void s_es_set_flag(dap_context_t *a_c, OVERLAPPED *a_ol) {
    ...
    if (a_ol->OffsetHigh) {
        ...
        case DAP_SOCK_READY_TO_READ:
            debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Set READ flag on es "DAP_FORMAT_ESOCKET_UUID, a_es->uuid);
            dap_events_socket_set_readable_unsafe_ex(a_es, true, NULL);
        break;
        case DAP_SOCK_READY_TO_WRITE:
            debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Set WRITE flag on es "DAP_FORMAT_ESOCKET_UUID, a_es->uuid);
            dap_events_socket_set_writable_unsafe_ex(a_es, true, 0, NULL);
        break;
        case DAP_SOCK_SIGNAL_CLOSE:
            debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Set CLOSE flag on es "DAP_FORMAT_ESOCKET_UUID, a_es->uuid);
            dap_events_socket_remove_and_delete_unsafe(a_es, false);
        break;
        default:
            debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Set flag %u on es "DAP_FORMAT_ESOCKET_UUID, flag, a_es->uuid);
            a_es->flags |= flag;
        }
    } else
        a_es->flags &= ~flag;
...
#endif
    debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Release es %p \"%s\" uuid " DAP_FORMAT_ESOCKET_UUID,
...
dap_events_socket_t * dap_events_socket_create(dap_events_desc_type_t a_type, dap_events_socket_callbacks_t* a_callbacks)
{
    ...
    l_es->type = a_type;
    debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Created socket %"DAP_FORMAT_SOCKET" type %d", l_sock,l_es->type);
    return l_es;
}
...
int dap_events_socket_connect(dap_events_socket_t *a_es, int *a_err_out)
{
    ...
    *a_err_out = 0;
    ...
    if (l_err == WSAEWOULDBLOCK || l_err == WSAEINPROGRESS)
        return 0;
    *a_err_out = l_err;
    return -1;
#else
    ...
    if (a_esocket->callbacks.queue_callback){
        if (a_esocket->flags & DAP_SOCK_QUEUE_PTR){
            ...
            if (l_ret == -1) {
                switch (errno) {
                case EAGAIN:
                    debug_if(dap_events_debug_reactor_get(), L_INFO, "Received and processed %lu callbacks in 1 pass", l_shift / 8);
                    break;
                default:
            ...
            if(l_read > 0) {
                debug_if(dap_events_debug_reactor_get(), L_NOTICE, "Got %ld bytes from socket", l_read);
            ...
            debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Dequeued %u items from "DAP_FORMAT_ESOCKET_UUID, l_count, a_esocket->uuid);
#else
...
static dap_queue_buf_thread_state_t *s_queue_buf_thread_state_find_unsafe(dap_events_socket_t *a_es)
{
    for (dap_queue_buf_thread_state_t *l_state = s_queue_buf_threads; l_state; l_state = l_state->next)
        if (l_state->es == a_es)
            return l_state;
    return NULL;
}

static dap_queue_buf_thread_state_t *s_queue_buf_thread_state_create_unsafe(dap_events_socket_t *a_es)
{
    dap_queue_buf_thread_state_t *l_state = DAP_NEW_Z(dap_queue_buf_thread_state_t);
    if (!l_state)
        return NULL;
    l_state->es = a_es;
    atomic_init(&l_state->running, false);
    atomic_init(&l_state->stop_requested, false);
    l_state->next = s_queue_buf_threads;
    s_queue_buf_threads = l_state;
    return l_state;
}

static void s_queue_buf_thread_state_delete_unsafe(dap_queue_buf_thread_state_t *a_state)
{
    if (!a_state)
        return;
    dap_queue_buf_thread_state_t **l_link = &s_queue_buf_threads;
    while (*l_link) {
        if (*l_link == a_state) {
            *l_link = a_state->next;
            DAP_DELETE(a_state);
...
{
    dap_queue_buf_thread_state_t *l_state = (dap_queue_buf_thread_state_t *)arg;
    dap_events_socket_t *l_es = l_state ? l_state->es : NULL;
    if (!l_es) {
...
#endif
    while (1) {
        if (atomic_load_explicit(&l_state->stop_requested, memory_order_acquire))
            break;
        pthread_rwlock_wrlock(&l_es->buf_out_lock);
        if (!l_es->buf_out_size || atomic_load_explicit(&l_state->stop_requested, memory_order_acquire)) {
        ...
        if (l_write_ret == -1) {
            ...
            case EAGAIN:
                pthread_rwlock_unlock(&l_es->buf_out_lock);
                struct timeval l_tv = { .tv_usec = 100 * 1000 };
                fd_set l_outfd; FD_ZERO(&l_outfd);
                FD_SET(l_sock, &l_outfd);
                sched_yield();
                int l_select_ret = select(l_sock + 1, NULL, &l_outfd, NULL, &l_tv);
                if (atomic_load_explicit(&l_state->stop_requested, memory_order_acquire))
                    break;
                if (l_select_ret == 0 || (l_select_ret == -1 && errno == EINTR))
                    continue;
                if (l_select_ret == -1) {
                    pthread_rwlock_wrlock(&l_es->buf_out_lock);
            ...
            debug_if(dap_events_debug_reactor_get(), L_DEBUG, "[!] Sent all %zd bytes to pipe [es %d]", l_write_ret, l_sock);
            l_es->buf_out_size = 0;
            pthread_rwlock_unlock(&l_es->buf_out_lock);
            break;
        } else if (l_write_ret) {
            debug_if(dap_events_debug_reactor_get(), L_DEBUG, "[!] Sent %zu / %zu bytes to pipe [es %d]", l_write_ret, l_es->buf_out_size, l_sock);
            l_es->buf_out_size -= l_write_ret;
            memmove(l_es->buf_out, l_es->buf_out + l_write_ret, l_es->buf_out_size);
        }

        if (l_write_ret > 0 && l_write_ret % sizeof(arg))
            log_it(L_CRITICAL, "[!] Sent unaligned chunk [%zd bytes] to pipe, possible data corruption!", l_write_ret);
        pthread_rwlock_unlock(&l_es->buf_out_lock);
    }
lb_thread_exit:
    atomic_store_explicit(&l_state->running, false, memory_order_release);
    pthread_exit(NULL);
}
...
static void s_add_ptr_to_buf(dap_events_socket_t * a_es, void* a_arg)
{
    ...
    bool l_need_start_thread = false;
    ...
    if (!a_es->buf_out_size) {
        ...
        byte_t *l_realloc = DAP_REALLOC(a_es->buf_out, a_es->buf_out_size_max);
        if (!l_realloc) {
            log_it(L_ERROR, "Can't increase queue buffer for es %p", a_es);
            pthread_rwlock_unlock(&a_es->buf_out_lock);
            return;
        }
        a_es->buf_out = l_realloc;
        debug_if(dap_events_debug_reactor_get(), L_MSG, "Es %p (%d): increase capacity to %zu, actual size: %zu",
        ...
        if (l_realloc)
            a_es->buf_out = l_realloc;
        debug_if(dap_events_debug_reactor_get(), L_MSG, "Es %p (%d): decrease capacity to %zu, actual size: %zu",
               a_es, a_es->fd, a_es->buf_out_size_max, a_es->buf_out_size);
    }
    *(void**)(a_es->buf_out + a_es->buf_out_size) = a_arg;
    a_es->buf_out_size += sizeof(a_arg);
    l_need_start_thread = true;
    pthread_rwlock_unlock(&a_es->buf_out_lock);

    if (!l_need_start_thread || (a_es->flags & DAP_SOCK_SIGNAL_CLOSE))
        return;

    static atomic_uint_fast64_t l_thd_count;
    while (1) {
        pthread_t l_thread_id = 0;
        bool l_need_join = false;
        bool l_started = false;
        pthread_mutex_lock(&s_queue_buf_threads_lock);
        dap_queue_buf_thread_state_t *l_state = s_queue_buf_thread_state_find_unsafe(a_es);
        if (!l_state)
            l_state = s_queue_buf_thread_state_create_unsafe(a_es);
        if (!l_state) {
            pthread_mutex_unlock(&s_queue_buf_threads_lock);
            log_it(L_ERROR, "Can't allocate queue buffer thread state for es %p", a_es);
            return;
        }
        if (l_state->destroy_requested || (a_es->flags & DAP_SOCK_SIGNAL_CLOSE)) {
            pthread_mutex_unlock(&s_queue_buf_threads_lock);
            return;
        }
        if (l_state->thread_created) {
            if (atomic_load_explicit(&l_state->running, memory_order_acquire)) {
                pthread_mutex_unlock(&s_queue_buf_threads_lock);
                return;
            }
            l_thread_id = l_state->thread_id;
            l_state->thread_created = false;
            l_need_join = true;
            pthread_mutex_unlock(&s_queue_buf_threads_lock);
            pthread_join(l_thread_id, NULL);
            continue;
        }
        atomic_store_explicit(&l_state->stop_requested, false, memory_order_release);
        atomic_store_explicit(&l_state->running, true, memory_order_release);
        int l_rc = pthread_create(&l_state->thread_id, NULL, s_dap_events_socket_buf_thread, l_state);
        if (l_rc == 0) {
            l_state->thread_created = true;
            l_thread_id = l_state->thread_id;
            l_started = true;
        } else {
            atomic_store_explicit(&l_state->running, false, memory_order_release);
            log_it(L_ERROR, "[#%"DAP_UINT64_FORMAT_U"] Cannot start thread, drop a_es: %p, a_arg: %p, rc: %d",
                   atomic_load(&l_thd_count) + 1, a_es, a_arg, l_rc);
        }
        pthread_mutex_unlock(&s_queue_buf_threads_lock);
        ...
        if (l_need_join)
            continue;
        if (l_started) {
            uint64_t l_count = atomic_fetch_add(&l_thd_count, 1) + 1;
            debug_if(dap_events_debug_reactor_get(), L_DEBUG, "[#%"DAP_UINT64_FORMAT_U"] Created thread %"DAP_UINT64_FORMAT_x", a_es: %p, a_arg: %p",
                     l_count, (uint64_t)l_thread_id, a_es, a_arg);
        }
        return;
    }
}

static void s_queue_buf_thread_stop_and_join(dap_events_socket_t *a_es)
{
    if (!a_es)
        return;
    ...
    l_state->destroy_requested = true;
    atomic_store_explicit(&l_state->stop_requested, true, memory_order_release);
    if (l_state->thread_created) {
        l_thread_id = l_state->thread_id;
        l_state->thread_created = false;
        l_need_join = true;
    } else {
        s_queue_buf_thread_state_delete_unsafe(l_state);
    }
    pthread_mutex_unlock(&s_queue_buf_threads_lock);
    ...
    pthread_mutex_lock(&s_queue_buf_threads_lock);
    l_state = s_queue_buf_thread_state_find_unsafe(a_es);
    if (l_state)
        s_queue_buf_thread_state_delete_unsafe(l_state);
    pthread_mutex_unlock(&s_queue_buf_threads_lock);
}
...
void dap_events_socket_remove_and_delete_unsafe( dap_events_socket_t *a_es, bool preserve_inheritor )
{
    assert(a_es);
    a_es->flags |= DAP_SOCK_SIGNAL_CLOSE;
    debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Remove es %p [%s] \"%s\" uuid "DAP_FORMAT_ESOCKET_UUID"",
    ...
    switch (l_res) {
    case 0:
        debug_if(dap_events_debug_reactor_get(), L_DEBUG, "\"%s\" on es "DAP_FORMAT_ESOCKET_UUID" completed immediately", func, a_es->uuid);
    break;
    case ERROR_IO_PENDING:
        debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Pending \"%s\" on es "DAP_FORMAT_ESOCKET_UUID, func, a_es->uuid);
        return;
    default:
        debug_if(dap_events_debug_reactor_get(), L_DEBUG, "\"%s\" on es "DAP_FORMAT_ESOCKET_UUID" failed, error %d: \"%s\"",
                                           func, a_es->uuid, l_res, dap_strerror(l_res));
        dap_events_socket_descriptor_close(a_es);
        return;
    }
    debug_if(dap_events_debug_reactor_get() && FLAG_KEEP_INHERITOR(a_es->flags), L_DEBUG, "Keep inheritor of "DAP_FORMAT_ESOCKET_UUID, a_es->uuid);
...
void dap_events_socket_set_readable_unsafe_ex(dap_events_socket_t *a_es, bool a_is_ready, dap_overlapped_t *a_ol) {
    if (a_es->flags & DAP_SOCK_SIGNAL_CLOSE) {
        debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Attempt to %sset read flag on closed socket %p, dump it",
                                           a_is_ready ? "" : "un", a_es);
    ...
    if (a_es->pending_read) {
        debug_if( dap_events_debug_reactor_get(), L_DEBUG, DAP_FORMAT_ESOCKET_UUID" : %zu \"%s\" already has pending read, dump it",
    ...
    case ERROR_SUCCESS:
    case ERROR_IO_PENDING:
        debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Pending \"%s\" on [%s] "DAP_FORMAT_ESOCKET_UUID,
...
{
    if (a_es->flags & DAP_SOCK_SIGNAL_CLOSE) {
        debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Attempt to %sset write flag on closed socket %p, dump it",
                                           a_is_ready ? "" : "un", a_es);
        return dap_overlapped_free(a_ol);
    }
    if (!a_is_ready) {
...
int dap_events_socket_queue_ptr_send( dap_events_socket_t *a_es, void *a_arg)
{
    ...
    if (!mq_send(a_es->mqd, (const char*)&a_arg, sizeof(a_arg), 0)) {
        debug_if (dap_events_debug_reactor_get(), L_DEBUG,"Sent ptr %p to esocket queue %p (%d)", a_arg, a_es, a_es? a_es->fd : -1);
        return 0;
    }
    ...
    if(a_es->pipe_out){ // If we have pipe out - we send events directly to the pipe out kqueue fd
        if(a_es->pipe_out->context){
            if( dap_events_debug_reactor_get()) log_it(L_DEBUG, "Sent kevent() with ptr %p to pipe_out worker on esocket %p",a_arg,a_es);
            l_n = kevent(a_es->pipe_out->context->kqueue_fd,&l_event,1,NULL,0,NULL);
        }
        else {
            log_it(L_WARNING,"Trying to send pointer in pipe out queue thats not assigned to any worker or proc thread");
        ...
        l_n = kevent(a_es->context->kqueue_fd,&l_event,1,NULL,0,NULL);
        if( dap_events_debug_reactor_get()) log_it(L_DEBUG, "Sent kevent() with ptr %p to worker on esocket %p",a_arg,a_es);
    }else {
...
void dap_events_socket_delete_unsafe(dap_events_socket_t *a_esocket, bool a_preserve_inheritor)
{
    ...
    if (a_esocket->type == DESCRIPTOR_TYPE_QUEUE) {
        s_queue_ops_wait_drain();
        s_queue_buf_thread_stop_and_join(a_esocket);
        if (a_esocket->cb_buf_cleaner) {
            if (a_esocket->buf_out_size) {
                a_esocket->cb_buf_cleaner((char *)a_esocket->buf_out, a_esocket->buf_out_size);
                a_esocket->buf_out_size = 0;
            }
            if (a_esocket->fd > 0) {
                void *l_items[DAP_QUEUE_MAX_MSGS];
                ssize_t l_read = 0;
                while ( (l_read = read(a_esocket->fd, l_items, sizeof(l_items))) > 0 )
                    a_esocket->cb_buf_cleaner((char *)l_items, (size_t)l_read);
            }
        }
        pthread_rwlock_destroy(&a_esocket->buf_out_lock);
    }
#endif
 ...
 */
#ifndef DAP_EVENTS_CAPS_IOCP
static atomic_uint_fast32_t s_queue_ops_inflight = 0;

static inline void s_queue_ops_enter(void)
{
    atomic_fetch_add_explicit(&s_queue_ops_inflight, 1, memory_order_acq_rel);
}

static inline void s_queue_ops_leave(void)
{
    atomic_fetch_sub_explicit(&s_queue_ops_inflight, 1, memory_order_acq_rel);
}

static void s_queue_ops_wait_drain(void)
{
    while (atomic_load_explicit(&s_queue_ops_inflight, memory_order_acquire))
        sched_yield();
}

static bool s_worker_queue_ready(const dap_worker_t *a_worker, const dap_events_socket_t *a_queue)
{
    return a_worker && a_queue;
}
#endif
...
void dap_events_socket_remove_and_delete(dap_worker_t *a_worker, dap_events_socket_uuid_t a_es_uuid)
{
...
#else
    dap_events_socket_t *l_queue = a_worker->queue_es_delete;
    ...
    *l_es_uuid_ptr = a_es_uuid;
    ...
    if( dap_events_socket_queue_ptr_send(l_queue, l_es_uuid_ptr) != 0 ){
...
size_t dap_events_socket_write_unsafe(dap_events_socket_t *a_es, const void *a_data, size_t a_data_size)
{
    ...
    if (a_es->flags & DAP_SOCK_SIGNAL_CLOSE) {
        debug_if(dap_events_debug_reactor_get(), L_NOTICE, "Trying to write into closing socket %"DAP_FORMAT_SOCKET, a_es->fd);
        return 0;
    }
    if (dap_events_debug_reactor_get() && a_es->context && dap_worker_get_current() != DAP_WORKER(a_es->context)) {
    ...
    if (a_es->buf_out_size_max < a_es->buf_out_size + a_data_size) {
        ...
        debug_if(dap_events_debug_reactor_get(), L_MSG, "[!] Socket %"DAP_FORMAT_SOCKET": increase capacity to %zu, actual size: %zu", a_es->fd, a_es->buf_out_size_max, a_es->buf_out_size);
        ...
        debug_if(dap_events_debug_reactor_get(), L_MSG, "[!] Socket %"DAP_FORMAT_SOCKET": decrease capacity to %zu, actual size: %zu",
    ...
    debug_if(dap_events_debug_reactor_get(), L_DEBUG, "Write %zu bytes to \"%s\" "DAP_FORMAT_ESOCKET_UUID", total size: %zu",
