...
/**
 * @file dap_hash_shake128.c
 * @brief SHAKE128 and cSHAKE128 implementation
 */
...
// =============================================================================
// cSHAKE128 Implementation (NIST SP 800-185)
// =============================================================================
...
static void s_cshake128_absorb_customization(dap_hash_keccak_ctx_t *a_ctx, uint16_t a_cstm)
{
    uint8_t l_buf[DAP_KECCAK_SHAKE128_RATE];
    size_t l_pos = 0;
    ...
    // left_encode(rate)
    l_pos += s_left_encode(l_buf + l_pos, DAP_KECCAK_SHAKE128_RATE);
    ...
    while (l_pos % DAP_KECCAK_SHAKE128_RATE != 0) {
        l_buf[l_pos++] = 0x00;
    }
                                  ...
                                  const uint8_t *a_input, size_t a_inlen)
{
    ...
    dap_hash_keccak_sponge_init(&l_ctx, DAP_KECCAK_SHAKE128_RATE, 0x04);
    
    // Absorb customization
    s_cshake128_absorb_customization(&l_ctx, a_cstm);
                          ...
                          const uint8_t *a_input, size_t a_inlen)
{
    ...
    // Squeeze output
    size_t l_nblocks = a_outlen / DAP_KECCAK_SHAKE128_RATE;
    dap_hash_cshake128_simple_squeezeblocks(a_output, l_nblocks, l_state);
    
    a_output += l_nblocks * DAP_KECCAK_SHAKE128_RATE;
    a_outlen -= l_nblocks * DAP_KECCAK_SHAKE128_RATE;
    
    if (a_outlen > 0) {
        uint8_t l_tmp[DAP_KECCAK_SHAKE128_RATE];
 ...
 * (e.g. deprecated tesla).  See dap_hash_shake128.h banner. */
                                       ...
                                       const uint8_t *a_input, size_t a_inlen)
{
    ...
    size_t l_nblocks = a_outlen / DAP_KECCAK_SHAKE128_RATE;
    if (l_nblocks)
        dap_hash_shake128_legacy_squeezeblocks(a_output, l_nblocks, l_state);
    ...
    if (a_outlen > 0) {
        uint8_t l_tmp[DAP_KECCAK_SHAKE128_RATE];
        dap_hash_shake128_legacy_squeezeblocks(l_tmp, 1, l_state);
        memcpy(a_output, l_tmp, a_outlen);
    }
}
