...
static uint64_t s_multi_sign_calc_size(const dap_multi_sign_t *a_sign, uint64_t *a_signes_size, uint64_t *a_pkeys_hashes_size)
{
    ...
    uint64_t l_pkeys_hashes_size = a_sign->key_count * sizeof(dap_hash_t);
...
// addtional allocation memory
    ...
    l_sign->key_hashes = DAP_NEW_Z_COUNT_RET_VAL_IF_FAIL(dap_hash_t, l_pkeys_hashes_size, NULL, l_sign->meta, l_sign->key_seq, l_sign);
 ...
 */
bool dap_multi_sign_hash_data(dap_multi_sign_t *a_sign, const void *a_data, const size_t a_data_size, dap_hash_t *a_hash)
{
    ...
    uint8_t l_meta_data[l_meta_data_size], l_concatenated_hash[ 3 * sizeof(dap_hash_t) ] = { };
    ...
    l_ret ? l_ret &= dap_hash_sha3_256(a_data, a_data_size, (dap_hash_t *)l_concatenated_hash) : 0;  // get data hash
    l_ret ? l_ret &= dap_hash_sha3_256(l_meta_data, l_meta_data_size, (dap_hash_t*)(l_concatenated_hash + sizeof(dap_hash_t))) : 0;  // get metadata hash
    l_ret ? l_ret &= dap_hash_sha3_256(a_sign->key_hashes, a_sign->key_count * sizeof(dap_hash_t), (dap_hash_t *)(l_concatenated_hash + 2 * sizeof(dap_hash_t))) : 0;   // get key_hashes hash
    l_ret ? l_ret &= dap_hash_sha3_256(l_concatenated_hash, 3 * sizeof(dap_hash_t), a_hash) : 0;  // get out hash of calculated hashes
    return l_ret;
}
...
// memory alloc
    ...
    l_sign->key_hashes = DAP_NEW_Z_COUNT_RET_VAL_IF_FAIL(dap_hash_t, l_params->key_count, -6);
...
// data prepare
    ...
    for (uint8_t i = 0; i < l_params->sign_count; ++i) {
        ...
        dap_sign_t *l_dap_sign_step = dap_sign_create(l_params->keys[l_num], &l_data_hash, sizeof(dap_hash_t));
        if (!l_dap_sign_step) {
        ...
        const size_t a_sign_size)
{
    ...
    for (uint8_t i = 0; i < l_sign->sign_count; ++i) {
        ...
        l_verified = dap_sign_verify(l_step_sign, &l_data_hash, sizeof(dap_hash_t));
