...
DAP_DISPATCH_LOCAL(aes_cbc_encrypt, size_t,
        struct dap_enc_key *, const void *, size_t, void *, size_t);
DAP_DISPATCH_LOCAL(aes_cbc_decrypt, size_t,
        struct dap_enc_key *, const void *, size_t, void *, size_t);
static pthread_once_t s_aes_once = PTHREAD_ONCE_INIT;

static void s_aes_dispatch_init(void)
{
    DAP_DISPATCH_DEFAULT(aes_cbc_encrypt, dap_enc_iaes256_cbc_encrypt_fast);
    DAP_DISPATCH_DEFAULT(aes_cbc_decrypt, dap_enc_iaes256_cbc_decrypt_fast);
        ...
        size_t a_in_size, void **a_out)
{
    ...
    *a_out = DAP_NEW_SIZE(uint8_t, l_out_size);
    if (!*a_out)
        return 0;
    return aes_cbc_encrypt_ptr(a_key, a_in, a_in_size, *a_out, l_out_size);
}
        ...
        size_t a_in_size, void **a_out)
{
    ...
    *a_out = DAP_NEW_SIZE(uint8_t, l_out_size);
    if (!*a_out)
        return 0;
    return aes_cbc_decrypt_ptr(a_key, a_in, a_in_size, *a_out, l_out_size);
}
...
{
    s_ensure_init();
    return aes_cbc_encrypt_ptr(a_key, a_in, a_in_size, a_out, a_out_size);
}
...
{
    s_ensure_init();
    return aes_cbc_decrypt_ptr(a_key, a_in, a_in_size, a_out, a_out_size);
}
