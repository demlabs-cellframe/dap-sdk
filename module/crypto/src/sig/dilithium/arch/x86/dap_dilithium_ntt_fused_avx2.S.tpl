 ...
 *
 * Output is in standard (non-shuffled) element order — a de-shuffle pass
 * runs after levels2t7 for compatibility with existing invNTT/pointwise.
 *
...
.macro levels2t7 off
    ...
    /* ---- De-shuffle: restore standard element order ---- */

    /* Step 1: undo shuffle2 */
    shuffle2 9, 8, 1, 2
    shuffle2 7, 6, 13, 14
    shuffle2 5, 4, 9, 8
    shuffle2 3, 11, 7, 6

    /* Step 2: undo shuffle4 */
    shuffle4 1, 13, 3, 4
    shuffle4 2, 14, 5, 10
    shuffle4 9, 7, 1, 13
    shuffle4 8, 6, 2, 14

    /* Step 3: undo shuffle8 */
    shuffle8 3, 1, 6, 7
    shuffle8 5, 2, 8, 9
    shuffle8 4, 13, 3, 1
    shuffle8 10, 14, 5, 2
    ...
    vmovdqu     %ymm6,  256*\off+  0(%rdi)
    vmovdqu     %ymm8,  256*\off+ 32(%rdi)
    vmovdqu     %ymm3,  256*\off+ 64(%rdi)
    vmovdqu     %ymm5,  256*\off+ 96(%rdi)
    vmovdqu     %ymm7,  256*\off+128(%rdi)
    vmovdqu     %ymm9,  256*\off+160(%rdi)
    vmovdqu     %ymm1,  256*\off+192(%rdi)
    vmovdqu     %ymm2,  256*\off+224(%rdi)
.endm
...
.p2align 5
s_fwd_zqinv:
    .long            0,   1830765815,  -1929875198,  -1927777021,   1640767044,   1477910808,   1612161320,   1640734244
...
.p2align 5
s_fwd_zeta:
    .long            0,        25847,     -2608894,      -518909,       237124,      -777960,      -876248,       466468
