/*
 * dap_sign_chipmunk_mring.c — dap_sign bridge for MRNG (log-N threshold ring).
 */
...
#define LOG_TAG "dap_sign_chipmunk_mring"

/* Outer seed redraw when CRNG returns NORM_BOUND (inner loop is 2048 attempts). */
#define MRING_SEED_RETRIES 16u

static dap_sign_t *s_chipmunk_mring_create(
    ...
    dap_enc_key_t **a_ring_keys, size_t a_ring_size)
{
    if (!a_signer_keys || !a_ring_keys ||
        ...
        log_it(L_ERROR, "mring sign: invalid parameters "
    ...
    if (a_ring_size < CHIPMUNK_RING_RING_MINIMUM_ANON) {
        log_it(L_WARNING, "mring sign: ring size %zu below recommended "
    ...
    for (size_t i = 0; i < a_signers_count; ++i) {
        if (!a_signer_keys[i] ||
            ...
            log_it(L_ERROR, "mring sign: signer key[%zu] invalid", i);
            return NULL;
        }
    }
    for (size_t i = 0; i < a_ring_size; ++i) {
        if (!a_ring_keys[i] ||
            ...
            log_it(L_ERROR, "mring sign: ring key[%zu] invalid", i);
            return NULL;
        }
    }
    ...
    for (uint32_t l_attempt = 0;
         l_attempt < MRING_SEED_RETRIES && l_rc != CHIPMUNK_RING_OK;
         ++l_attempt) {
    ...
    if (l_rc != CHIPMUNK_RING_OK || !l_blob) {
        log_it(L_ERROR, "mring sign failed (N=%u, t=%u, rc=%d %s)",
...
static int s_chipmunk_mring_verify(dap_sign_t *a_sign,
                                   const void *a_data, size_t a_data_size,
                                   dap_enc_key_t **a_ring_keys, size_t a_ring_size)
{
    ...
    for (size_t i = 0; i < a_ring_size; ++i) {
        if (!a_ring_keys[i] ||
            !a_ring_keys[i]->pub_key_data ||
            a_ring_keys[i]->pub_key_data_size != DAP_ENC_CHIPMUNK_RING_PUB_KEY_SIZE) {
            log_it(L_ERROR, "mring verify: ring key[%zu] invalid", i);
            return -EINVAL;
        }
    }
    ...
    chipmunk_ring_error_t l_e = chipmunk_ring_verify_from_bytes(
        a_sign->pkey_n_sign, a_sign->header.sign_size, l_ring, l_N,
        (const uint8_t *)a_data, a_data_size, NULL, 0);

    DAP_DELETE(l_ring);

    if (l_e != CHIPMUNK_RING_OK) {
        log_it(L_INFO, "mring verify failed: %s",
...
{
    dap_sign_type_t l_type = { .type = SIG_TYPE_CHIPMUNK_MRING };
    return dap_sign_register_ring_callbacks(l_type,
                                            s_chipmunk_mring_create,
                                            s_chipmunk_mring_verify);
}
