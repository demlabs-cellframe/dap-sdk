...
typedef struct chipmunk_tree {
    ...
    uint8_t hasher_seed[32];                    ///< Seed of the HVC hasher that produced non-leaf nodes (CR-D15.A)
} chipmunk_tree_t;
 ...
 * @details Honest Ajtai lattice hash h(x, y) = Σ_i A_left[i]·D(x)[i] + A_right[i]·D(y)[i]
 *          where D is a signed-digit base-(2·ZETA+1)=59 decomposition of a
 *          HVC polynomial into HVC_WIDTH smaller-norm pieces, and A_left /
 *          A_right are uniform random polynomials in R_q = Z_q[X]/(X^N+1)
 *          derived from the hasher seed through SHAKE128.
 *
 * CR-D15.A: replaces the previous stub where the matrix was generated by an
 * ad-hoc LCG over the first seed byte and the "hash" was plain coefficient-
 * wise addition — both of which trivialised collision resistance and made
 * the full tree reducible to a single XOR.
 */
typedef struct chipmunk_hvc_hasher {
    // CR-D15.A performance optimisation: the public matrices are kept in NTT
    // form after hasher_init, so the hot path (`chipmunk_hvc_hash_decom_then_hash`)
    // only has to forward-NTT the two decomposed inputs and reverse-NTT the
    // accumulator once, saving W forward NTTs per hash (~3x speedup on 100-
    // signer Merkle trees).  The plain-coefficient form is not materialised
    // anywhere because no caller needs it outside the hash primitive.
    int32_t matrix_a_left_ntt[CHIPMUNK_HVC_WIDTH][CHIPMUNK_N];
    int32_t matrix_a_right_ntt[CHIPMUNK_HVC_WIDTH][CHIPMUNK_N];
    uint8_t seed[32];                                         ///< Seed for matrix derivation
 ...
 *
 * CR-D15.A (Round-5): the previous implementation only verified the FIRST
 * level of the proof and contained an explicit `TODO` — every intermediate
 * level was silently accepted.  A tampered path whose first pair happened
 * to hash to the right root would verify regardless of its interior nodes.
 *
 * The corrected algorithm walks the path bottom-up, checking every level:
 *   1. the leaf must occupy the side of path[0] dictated by index parity,
 *   2. hash(path[i].left, path[i].right) must appear in path[i+1] on the
 *      side dictated by `(index >> (i+1)) & 1`,
 *   3. the root-level hash of the final pair must equal the stored root.
 *
 * @param[in] a_path    Proof path (top-to-bottom order, index encodes parity)
 * @param[in] a_leaf    Leaf digest claimed by the prover (MUST be pinned by
 *                      the caller to the HOTS public key it represents)
 * @param[in] a_root    Expected tree root
 * @param[in] a_hasher  HVC hasher used by the tree that produced the proof
 * @return true iff every level verifies and the final hash equals a_root.
 */
