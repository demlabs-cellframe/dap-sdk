...
enum chipmunk_error_t {
    ...
    /* CR-D3 fix: Chipmunk is built on HOTS, which is existentially forgeable
     * after a SECOND signing under the same secret key — the attacker can
     * recover both s0 and s1 from two signatures on different messages.
     * chipmunk_sign enforces strict single-use via a leaf_index counter
     * embedded in the private key buffer; once the counter reaches
     * CHIPMUNK_MAX_SIGNATURES the key is refused for any further signature
     * and the caller MUST rotate to a fresh keypair (or, after CR-D15 ships,
     * advance to the next Merkle leaf). */
    CHIPMUNK_ERROR_KEY_EXHAUSTED = -12 ///< One-time HOTS key already consumed
};
...
/* CR-D3 fix: 4 leading bytes of the private-key buffer carry a big-endian
 * monotonic leaf-index counter.  Each successful chipmunk_sign bumps it and
 * rejects any further signing attempt once the counter hits
 * CHIPMUNK_MAX_SIGNATURES.  This closes the classic HOTS key-recovery
 * attack where two signatures on different messages leak s0 and s1. */
#define CHIPMUNK_LEAF_INDEX_SIZE 4u
#define CHIPMUNK_PRIVATE_KEY_SIZE (CHIPMUNK_LEAF_INDEX_SIZE + 32 + 48 + CHIPMUNK_PUBLIC_KEY_SIZE) // leaf_index + key_seed + tr + public_key

/* Strict one-time regime for plain Chipmunk HOTS.  CR-D15 will raise this
 * to (1u << CHIPMUNK_HEIGHT) once per-leaf Merkle authentication paths are
 * added to signatures so that multiple leaves can be consumed safely. */
#define CHIPMUNK_MAX_SIGNATURES 1u
 ...
 *
 * CR-D3 fix: carries a monotonic leaf_index counter that is persisted inside
 * the serialized private-key buffer.  chipmunk_sign strictly refuses to sign
 * when leaf_index >= CHIPMUNK_MAX_SIGNATURES; this prevents the classical
 * HOTS two-message key-recovery attack.  The counter is serialized
 * big-endian as the first 4 bytes of the private-key bytes.
 */
typedef struct chipmunk_private_key {
    uint32_t leaf_index;     ///< Monotonic one-time counter (one sig per keypair)
 ...
 *
 * CR-D3 fix: the private-key buffer carries a monotonic leaf_index counter
 * that is advanced on every successful signature.  The buffer is therefore
 * MUTATED by this call — callers must treat the buffer as both input and
 * output and persist the updated bytes if they need the counter to survive
 * across processes.  Once the counter reaches CHIPMUNK_MAX_SIGNATURES the
 * function returns CHIPMUNK_ERROR_KEY_EXHAUSTED and the caller MUST rotate
 * to a fresh keypair; re-signing with an exhausted key would expose s0 and
 * s1 to anyone who sees both signatures.
 *
 * @param[in,out] a_private_key Private key buffer (leaf_index is updated)
