 ...
 *          For outputs > 32 bytes, uses counter mode expansion (HKDF-like).
 *          Supports optional salt and iterative hashing for key stretching.
 *
 * Security properties:
                        ...
                        uint32_t a_iterations)
{
    ...
    // Phase 2: Key stretching via iterative hashing (optional)
    uint32_t iterations = a_iterations > 0 ? a_iterations : 1;
    for (uint32_t i = 1; i < iterations; i++) {
        dap_hash(DAP_HASH_TYPE_SHA3_256, prk, sizeof(prk), prk, sizeof(prk));
    }
    
    // Phase 3: Output expansion using counter mode (HKDF-Expand style)
    // OKM = T(1) || T(2) || ... where T(i) = SHA3-256(PRK || T(i-1) || counter)
    ...
    while (remaining > 0) {
        // Expand input: PRK || prev_block (except first iteration) || counter
