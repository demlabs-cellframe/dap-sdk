...
void hash_vm(unsigned char *c_bin, poly_k *v, const unsigned char *m, unsigned long long mlen, tesla_param_t *p)
{
    ...
    /* Deprecated tesla: legacy SHAKE squeeze for master compat */
    if(p->kind == 0 || p->kind == 3)
        dap_hash_shake128_legacy(c_bin, CRYPTO_C_BYTES, t, p->PARAM_K * p->PARAM_N + mlen);
    else
        dap_hash_shake256_legacy(c_bin, CRYPTO_C_BYTES, t, p->PARAM_K * p->PARAM_N + mlen);
...
int tesla_crypto_sign_keypair(tesla_public_key_t *public_key, tesla_private_key_t *private_key, tesla_kind_t kind, const void * seed, size_t seed_size)
{
    ...
    if(seed && seed_size>0){
        ...
        dap_random_bytes(randomness, CRYPTO_RANDOMBYTES);
    }

    if(p->kind == 0 || p->kind == 3)
        dap_hash_shake128_legacy(randomness_extended, ((p->PARAM_K) + 3) * CRYPTO_SEEDBYTES, randomness, CRYPTO_RANDOMBYTES);
    else
        dap_hash_shake256_legacy(randomness_extended, ((p->PARAM_K) + 3) * CRYPTO_SEEDBYTES, randomness, CRYPTO_RANDOMBYTES);
...
{    
    ...
    dap_random_bytes(randomness_input + CRYPTO_RANDOMBYTES, CRYPTO_RANDOMBYTES);
    ...
    if(p->kind == 0 || p->kind == 3)
        dap_hash_shake128_legacy(randomness, CRYPTO_SEEDBYTES, randomness_input, CRYPTO_RANDOMBYTES + CRYPTO_SEEDBYTES + mlen);
    else
        dap_hash_shake256_legacy(randomness, CRYPTO_SEEDBYTES, randomness_input, CRYPTO_RANDOMBYTES + CRYPTO_SEEDBYTES + mlen);
