...
static dap_json_bitmaps_avx2_t s_classify_chunk_avx2(const uint8_t *a_chunk)
{
    dap_json_bitmaps_avx2_t bitmaps = {0};
    
    // Load chunk
    VECTOR_TYPE chunk = _mm256_loadu_si256((const VECTOR_TYPE *)a_chunk);
    
    // Create comparison vectors
    VECTOR_TYPE v_space = _mm256_set1_epi8(' ');
    VECTOR_TYPE v_tab = _mm256_set1_epi8('\t');
    VECTOR_TYPE v_cr = _mm256_set1_epi8('\r');
    VECTOR_TYPE v_lf = _mm256_set1_epi8('\n');
    VECTOR_TYPE v_quote = _mm256_set1_epi8('"');
    VECTOR_TYPE v_backslash = _mm256_set1_epi8('\\');
    VECTOR_TYPE v_op_brace = _mm256_set1_epi8('{');
    VECTOR_TYPE v_cl_brace = _mm256_set1_epi8('}');
    VECTOR_TYPE v_op_bracket = _mm256_set1_epi8('[');
    VECTOR_TYPE v_cl_bracket = _mm256_set1_epi8(']');
    VECTOR_TYPE v_colon = _mm256_set1_epi8(':');
    VECTOR_TYPE v_comma = _mm256_set1_epi8(',');
    

    // SSE2/AVX2/NEON: Vector comparisons + movemask
    VECTOR_TYPE whitespace = _mm256_or_si256(
    ...
    VECTOR_TYPE quote = _mm256_cmpeq_epi8(chunk, v_quote);
    VECTOR_TYPE backslash = _mm256_cmpeq_epi8(chunk, v_backslash);
    
    VECTOR_TYPE structural = _mm256_or_si256(
        _mm256_or_si256(
    ...
    // Convert to bitmasks
    bitmaps.whitespace = (MASK_TYPE)_mm256_movemask_epi8(whitespace);
    bitmaps.quote = (MASK_TYPE)_mm256_movemask_epi8(quote);
    bitmaps.backslash = (MASK_TYPE)_mm256_movemask_epi8(backslash);
    bitmaps.structural = (MASK_TYPE)_mm256_movemask_epi8(structural);
 ...
 */
static inline int s_add_token_avx2(
 ...
 * 
 * Performance: 4-5 GB/s (single-core)
 */
...
int dap_json_stage1_run_avx2(dap_json_stage1_t *a_stage1)
{
    ...
    while (pos < chunk_end) {
        ...
        dap_json_bitmaps_avx2_t bitmaps = s_classify_chunk_avx2(input + pos);
        ...
        while (chunk_pos < chunk_limit) {
            ...
            if (bit_offset < CHUNK_SIZE && (bitmaps.structural & (((MASK_TYPE)1) << bit_offset))) {
                int ret = s_add_token_avx2(a_stage1, (uint32_t)chunk_pos, 0, TOKEN_TYPE_STRUCTURAL, c);
            ...
            if (c == '"') {
                ...
                int ret = s_add_token_avx2(a_stage1, (uint32_t)chunk_pos, (uint32_t)str_len,
                                     TOKEN_TYPE_STRING, 0);
                if (ret != STAGE1_SUCCESS) return ret;
            ...
            if (c == '-' || (c >= '0' && c <= '9')) {
                ...
                int ret = s_add_token_avx2(a_stage1, (uint32_t)chunk_pos, (uint32_t)num_len,
                                     TOKEN_TYPE_NUMBER, 0);
                if (ret != STAGE1_SUCCESS) return ret;
            ...
            if (c == 't' || c == 'f' || c == 'n') {
                ...
                int ret = s_add_token_avx2(a_stage1, (uint32_t)chunk_pos, (uint32_t)lit_len,
    ...
    while (pos < input_len) {
        ...
        // Structural
        if (c == '{' || c == '}' || c == '[' || c == ']' || c == ':' || c == ',') {
            int ret = s_add_token_avx2(a_stage1, (uint32_t)pos, 0, TOKEN_TYPE_STRUCTURAL, c);
            if (ret != STAGE1_SUCCESS) return ret;
            a_stage1->structural_chars++;
            pos++;
        }
        // String
            ...
            int ret = s_add_token_avx2(a_stage1, (uint32_t)pos, (uint32_t)str_len,
                                 TOKEN_TYPE_STRING, 0);
            if (ret != STAGE1_SUCCESS) return ret;
        ...
        // Number
            ...
            int ret = s_add_token_avx2(a_stage1, (uint32_t)pos, (uint32_t)num_len,
                                 TOKEN_TYPE_NUMBER, 0);
            if (ret != STAGE1_SUCCESS) return ret;
        ...
        // Literal
            ...
            int ret = s_add_token_avx2(a_stage1, (uint32_t)pos, (uint32_t)lit_len,
                                 TOKEN_TYPE_LITERAL, 0);
            if (ret != STAGE1_SUCCESS) return ret;
 ...
 */
void dap_json_stage1_avx2_set_debug(bool a_enable)
{
    // AVX2 now uses global dap_json_get_debug(), no local flag needed
    (void)a_enable;
}
