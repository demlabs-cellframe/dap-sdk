 ...
 * @brief SimdJSON-style Stage 1 tokenization with NEON SIMD optimization
...
static dap_json_bitmaps_neon_t s_classify_chunk_neon(const uint8_t *a_chunk)
{
    dap_json_bitmaps_neon_t bitmaps = {0};
    
    // Load chunk
    VECTOR_TYPE chunk = vld1q_u8((const VECTOR_TYPE *)a_chunk);
    
    // Create comparison vectors
    VECTOR_TYPE v_space = vdupq_n_u8(' ');
    VECTOR_TYPE v_tab = vdupq_n_u8('\t');
    VECTOR_TYPE v_cr = vdupq_n_u8('\r');
    VECTOR_TYPE v_lf = vdupq_n_u8('\n');
    VECTOR_TYPE v_quote = vdupq_n_u8('"');
    VECTOR_TYPE v_backslash = vdupq_n_u8('\\');
    VECTOR_TYPE v_op_brace = vdupq_n_u8('{');
    VECTOR_TYPE v_cl_brace = vdupq_n_u8('}');
    VECTOR_TYPE v_op_bracket = vdupq_n_u8('[');
    VECTOR_TYPE v_cl_bracket = vdupq_n_u8(']');
    VECTOR_TYPE v_colon = vdupq_n_u8(':');
    VECTOR_TYPE v_comma = vdupq_n_u8(',');
    

    // SSE2/AVX2/NEON: Vector comparisons + movemask
    VECTOR_TYPE whitespace = vorrq_u8(
        vorrq_u8(vceqq_u8(chunk, v_space), vceqq_u8(chunk, v_tab)),
        vorrq_u8(vceqq_u8(chunk, v_cr), vceqq_u8(chunk, v_lf))
    );
    
    VECTOR_TYPE quote = vceqq_u8(chunk, v_quote);
    VECTOR_TYPE backslash = vceqq_u8(chunk, v_backslash);
    
    VECTOR_TYPE structural = vorrq_u8(
        vorrq_u8(
            vorrq_u8(vceqq_u8(chunk, v_op_brace), vceqq_u8(chunk, v_cl_brace)),
            vorrq_u8(vceqq_u8(chunk, v_op_bracket), vceqq_u8(chunk, v_cl_bracket))
        ),
        vorrq_u8(vceqq_u8(chunk, v_colon), vceqq_u8(chunk, v_comma))
    );
    
    // Convert to bitmasks
    bitmaps.whitespace = (MASK_TYPE)dap_neon_movemask_u8(whitespace);
    bitmaps.quote = (MASK_TYPE)dap_neon_movemask_u8(quote);
    bitmaps.backslash = (MASK_TYPE)dap_neon_movemask_u8(backslash);
    bitmaps.structural = (MASK_TYPE)dap_neon_movemask_u8(structural);
 ...
 */
static inline int s_add_token_neon(
...
/* ========================================================================== */
/*                    MAIN TOKENIZATION - NEON                       */
/* ========================================================================== */

/**
 * @brief Full SIMD-optimized Stage 1 tokenization (NEON)
...
int dap_json_stage1_run_neon(dap_json_stage1_t *a_stage1)
{
    ...
    debug_if(dap_json_get_debug(), "Starting NEON SimdJSON Stage 1 tokenization (%zu bytes)", input_len);
    ...
    while (pos < chunk_end) {
        ...
        dap_json_bitmaps_neon_t bitmaps = s_classify_chunk_neon(input + pos);
        ...
        while (chunk_pos < chunk_limit) {
            ...
            if (bit_offset < CHUNK_SIZE && (bitmaps.structural & (((MASK_TYPE)1) << bit_offset))) {
                int ret = s_add_token_neon(a_stage1, (uint32_t)chunk_pos, 0, TOKEN_TYPE_STRUCTURAL, c);
            ...
            if (c == '"') {
                ...
                int ret = s_add_token_neon(a_stage1, (uint32_t)chunk_pos, (uint32_t)str_len,
                                     TOKEN_TYPE_STRING, 0);
                if (ret != STAGE1_SUCCESS) return ret;
            ...
            if (c == '-' || (c >= '0' && c <= '9')) {
                ...
                int ret = s_add_token_neon(a_stage1, (uint32_t)chunk_pos, (uint32_t)num_len,
                                     TOKEN_TYPE_NUMBER, 0);
                if (ret != STAGE1_SUCCESS) return ret;
            ...
            if (c == 't' || c == 'f' || c == 'n') {
                ...
                int ret = s_add_token_neon(a_stage1, (uint32_t)chunk_pos, (uint32_t)lit_len,
    ...
    while (pos < input_len) {
        ...
        // Structural
        if (c == '{' || c == '}' || c == '[' || c == ']' || c == ':' || c == ',') {
            int ret = s_add_token_neon(a_stage1, (uint32_t)pos, 0, TOKEN_TYPE_STRUCTURAL, c);
            if (ret != STAGE1_SUCCESS) return ret;
            a_stage1->structural_chars++;
            pos++;
        }
        // String
            ...
            int ret = s_add_token_neon(a_stage1, (uint32_t)pos, (uint32_t)str_len,
                                 TOKEN_TYPE_STRING, 0);
            if (ret != STAGE1_SUCCESS) return ret;
        ...
        // Number
            ...
            int ret = s_add_token_neon(a_stage1, (uint32_t)pos, (uint32_t)num_len,
                                 TOKEN_TYPE_NUMBER, 0);
            if (ret != STAGE1_SUCCESS) return ret;
        ...
        // Literal
            ...
            int ret = s_add_token_neon(a_stage1, (uint32_t)pos, (uint32_t)lit_len,
                                 TOKEN_TYPE_LITERAL, 0);
            if (ret != STAGE1_SUCCESS) return ret;
    ...
    debug_if(dap_json_get_debug(), "NEON SimdJSON Stage 1 complete: %zu tokens (%zu structural, %zu strings, %zu numbers, %zu literals)",
 ...
 */
void dap_json_stage1_neon_set_debug(bool a_enable)
{
    // NEON now uses global dap_json_get_debug(), no local flag needed
    (void)a_enable;
}
