...
// SIMD main loop - SVE predicate-based implementation
while (l_pos + l_vlen <= a_input_len) {
    ...
    // Compare for quotes and backslashes (returns predicates)
    SIMD_PRED_TYPE l_quotes_pred = SIMD_CMP_EQ(l_pred, l_chunk, l_quote_vec);
    SIMD_PRED_TYPE l_backslashes_pred = SIMD_CMP_EQ(l_pred, l_chunk, l_backslash_vec);
