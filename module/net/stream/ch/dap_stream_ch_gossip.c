...
void dap_gossip_msg_issue(dap_cluster_t *a_cluster, const char a_ch_id, const void *a_payload, size_t a_payload_size, dap_hash_sha3_256_t *a_payload_hash)
{
    ...
    HASH_VALUE(a_payload_hash, sizeof(dap_hash_sha3_256_t), l_hash_value);
    HASH_FIND_BYHASHVALUE(hh, s_gossip_last_msgs, a_payload_hash, sizeof(dap_hash_sha3_256_t), l_hash_value, l_msg_item);
    if (l_msg_item) {
        pthread_rwlock_unlock(&s_gossip_lock);
    ...
    *(dap_stream_node_addr_t *)l_msg->trace_n_payload = g_node_addr;
    ...
    HASH_ADD_BYHASHVALUE(hh, s_gossip_last_msgs, payload_hash, sizeof(dap_hash_sha3_256_t), l_hash_value, l_msg_item);
    pthread_rwlock_unlock(&s_gossip_lock);
...
static bool s_stream_ch_packet_in(dap_stream_ch_t *a_ch, void *a_arg)
{
    ...
    case DAP_STREAM_CH_GOSSIP_MSG_TYPE_REQUEST: {
        ...
        unsigned l_hash_value = 0;
        HASH_VALUE(l_payload_hash, sizeof(dap_hash_sha3_256_t), l_hash_value);
        pthread_rwlock_wrlock(&s_gossip_lock);
        HASH_FIND_BYHASHVALUE(hh, s_gossip_last_msgs, l_ch_pkt->data, sizeof(dap_hash_sha3_256_t), l_hash_value, l_msg_item);
        if (l_msg_item) {
            ...
            HASH_ADD_BYHASHVALUE(hh, s_gossip_last_msgs, payload_hash, sizeof(dap_hash_sha3_256_t), l_hash_value, l_item_new);
    ...
    case DAP_STREAM_CH_GOSSIP_MSG_TYPE_DATA: {
        ...
        unsigned l_hash_value = 0;
        HASH_VALUE(&l_msg->payload_hash, sizeof(dap_hash_sha3_256_t), l_hash_value);
        struct gossip_msg_item *l_payload_item = NULL, *l_payload_item_new;
        pthread_rwlock_wrlock(&s_gossip_lock);
        HASH_FIND_BYHASHVALUE(hh, s_gossip_last_msgs, &l_msg->payload_hash, sizeof(dap_hash_sha3_256_t), l_hash_value, l_payload_item);
        ...
        HASH_ADD_BYHASHVALUE(hh, s_gossip_last_msgs, payload_hash, sizeof(dap_hash_sha3_256_t), l_hash_value, l_payload_item);
