                                            ...
                                            const dap_net_trans_server_ops_t *a_ops)
{
    ...
    dap_ht_find_int(s_ops_registry, a_trans_type, l_entry);
    if (l_entry) {
    ...
    dap_ht_add(s_ops_registry, trans_type, l_entry);
    
    log_it(L_INFO, "Registered trans server operations for type %d (registry size: %u)", 
           a_trans_type, dap_ht_count(s_ops_registry));
...
void dap_net_trans_server_unregister_ops(dap_net_trans_type_t a_trans_type)
{
    ...
    dap_ht_find_int(s_ops_registry, a_trans_type, l_entry);
    if (l_entry) {
        dap_ht_del(s_ops_registry, l_entry);
        DAP_DELETE(l_entry);
...
int dap_net_trans_server_register_handlers(dap_net_trans_server_ctx_t *a_ctx)
{
    ...
    // dap_ht_find_str returns only the first entry for duplicate keys,
    // so we check if /stream is already handled by the trans-specific handler.
    dap_http_url_proc_t *l_existing_stream = NULL;
    dap_ht_find_str(a_ctx->http_server->url_proc, "/stream", l_existing_stream);
...
int dap_net_server_listen_addr_add_with_callback(dap_server_t *a_server,
                                                ...
                                                void *a_arg)
{
    (void)a_pre_worker_added;
    (void)a_arg;
    return dap_server_listen_addr_add(a_server, a_addr, a_port, a_type, NULL);
}
