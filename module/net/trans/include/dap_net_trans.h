...
/**
 * @brief Trans type enumeration — included from dap_net_trans_types.h
 */
#include "dap_net_trans_types.h"

#if 0 /* types already defined in dap_net_trans_types.h */
typedef enum dap_net_trans_type {
    ...
    DAP_NET_TRANS_MAX            = 0x07
} dap_net_trans_type_t;
#endif /* types already defined */
...
typedef struct dap_net_session_params {
    ...
    dap_enc_key_t *session_key;           ///< Session key (from handshake)
...
struct dap_net_trans {
    ...
    dap_ht_handle_t hh;                    ///< Hash table handle (keyed by type)
};
