...
#ifdef DAP_OS_WASM_MT
/* Signature must match the other declarations of this JS library and the
 * library itself (wasm-ld rejects a mismatch); 0 keeps the default budget. */
...
static int s_do_signaling(rtc_conn_t *a_conn)
{
    ...
    if (l_ice_args.result > 0 && l_ice_args.out) {
        ...
        js_http_post_sync(l_url, "application/json",
                          ...
                          NULL, (int)(uintptr_t)&l_ice_resp, (int)(uintptr_t)&l_ice_resp_len, 0);
