...
#endif
#include <ctype.h>
#include "dap_hash_sha1.h"
#include "dap_common.h"
#include "dap_strfuncs.h"
#include "dap_net_trans.h"
#include "dap_net_trans_websocket_server.h"
#include "dap_http_client.h"
#include "dap_http_header.h"  // dap_http_out_header_add is defined here
...
static inline char *s_strcasestr(const char *a_haystack, const char *a_needle)
{
    if (!a_needle[0]) return (char *)a_haystack;
    for (; *a_haystack; ++a_haystack) {
        const char *h = a_haystack, *n = a_needle;
        while (*h && *n && tolower((unsigned char)*h) == tolower((unsigned char)*n))
            ++h, ++n;
        if (!*n) return (char *)a_haystack;
    }
    return NULL;
}
#define strcasestr s_strcasestr
#endif
...
static int s_switch_to_websocket_protocol(dap_http_client_t *a_http_client)
{
    ...
    if (!l_stream) {
        dap_cluster_node_addr_t *l_node_addr = l_session ? &l_session->node : NULL;
...
static void s_ws_server_esocket_error(dap_events_socket_t *a_es, int a_arg)
{
    (void)a_arg;
    if (!a_es) return;
    log_it(L_ERROR, "WebSocket server: esocket error on fd=%" DAP_FORMAT_SOCKET, a_es->socket);
    a_es->flags |= DAP_SOCK_SIGNAL_CLOSE;
}
