...
const char *dap_http_method_to_str(dap_http_method_t a_method)
{
    switch (a_method) {
        case DAP_HTTP_METHOD_GET:     return "GET";
        case DAP_HTTP_METHOD_HEAD:   return "HEAD";
        case DAP_HTTP_METHOD_POST:   return "POST";
        case DAP_HTTP_METHOD_PUT:    return "PUT";
        case DAP_HTTP_METHOD_DELETE: return "DELETE";
        case DAP_HTTP_METHOD_OPTIONS: return "OPTIONS";
        case DAP_HTTP_METHOD_PATCH:  return "PATCH";
        default:                      return "GET";
    }
}

dap_http_method_t dap_http_method_from_str(const char *a_method)
{
    if (!a_method) return DAP_HTTP_METHOD_GET;
    if (strncasecmp(a_method, "GET", 3) == 0 && (a_method[3] == '\0' || a_method[3] == ' '))
        return DAP_HTTP_METHOD_GET;
    if (strncasecmp(a_method, "HEAD", 4) == 0 && (a_method[4] == '\0' || a_method[4] == ' '))
        return DAP_HTTP_METHOD_HEAD;
    if (strncasecmp(a_method, "POST", 4) == 0 && (a_method[4] == '\0' || a_method[4] == ' '))
        return DAP_HTTP_METHOD_POST;
    if (strncasecmp(a_method, "PUT", 3) == 0 && (a_method[3] == '\0' || a_method[3] == ' '))
        return DAP_HTTP_METHOD_PUT;
    if (strncasecmp(a_method, "DELETE", 6) == 0 && (a_method[6] == '\0' || a_method[6] == ' '))
        return DAP_HTTP_METHOD_DELETE;
    if (strncasecmp(a_method, "OPTIONS", 7) == 0 && (a_method[7] == '\0' || a_method[7] == ' '))
        return DAP_HTTP_METHOD_OPTIONS;
    if (strncasecmp(a_method, "PATCH", 5) == 0 && (a_method[5] == '\0' || a_method[5] == ' '))
...
/**
 * @brief Parse single HTTP header line "Name: Value" into name/value buffers (6.0 compat)
 * @return 0 on success, -1 on error, 1 if empty/CRLF-only line
 */
static int s_parse_header_line(const char *a_line, size_t a_len,
                               char *a_name, size_t a_name_size,
                               char *a_value, size_t a_value_size)
{
    if (!a_line || a_len < 2)
        return 1; /* empty */
    if (a_len >= 2 && a_line[0] == '\r' && a_line[1] == '\n')
        return 1; /* CRLF only */
    if (a_len < 4)
        return -1;

    const char *colon = memchr(a_line, ':', a_len);
    if (!colon)
        return -1;

    size_t name_len = (size_t)(colon - a_line);
    if (name_len >= a_name_size || name_len == 0)
        return -1;

    /* Copy name, trim trailing spaces */
    while (name_len > 0 && (a_line[name_len - 1] == ' ' || a_line[name_len - 1] == '\t'))
        name_len--;
    memcpy(a_name, a_line, name_len);
    a_name[name_len] = '\0';

    /* Value: skip colon and leading spaces */
    const char *val_start = colon + 1;
    size_t val_remaining = a_len - (val_start - a_line);
    while (val_remaining > 0 && (*val_start == ' ' || *val_start == '\t')) {
        val_start++;
        val_remaining--;
    }
    /* Trim trailing CRLF */
    while (val_remaining > 0 && (val_start[val_remaining - 1] == '\r' || val_start[val_remaining - 1] == '\n'))
        val_remaining--;
    if (val_remaining >= a_value_size)
        return -1;
    memcpy(a_value, val_start, val_remaining);
    a_value[val_remaining] = '\0';
    return 0;
}
...
static int s_parse_response_header(dap_client_http_t *a_client_http, const char *a_header_line, size_t a_header_len)
{
    ...
    int l_ret = s_parse_header_line(a_header_line, a_header_len,
    ...
    int *a_error_code)
{
    ...
    // Resolve host and set address size for connect()
    int l_addr_size = dap_net_resolve_host(a_uplink_addr, dap_itoa(a_uplink_port), false, &l_ev_socket->addr_storage, NULL);
    if (l_addr_size < 0) {
    ...
    l_ev_socket->addr_size = (socklen_t)l_addr_size;
