...
void dap_json_rpc_sign_get_information(dap_json_t* a_json_arr_reply, dap_sign_t* a_sign, dap_json_t *a_json_out, const char *a_hash_out_type, int a_version)
{
    ...
    if (dap_sign_get_pkey_hash(a_sign, &l_hash_pkey)) {
        if (dap_strcmp(a_hash_out_type, "hex")) {
            ...
            dap_json_object_add_string(a_json_out, a_version == 1 ? "Public key hash" : "pkey_hash", l_hash_b58.s);
        } else {
            dap_hash_sha3_256_str_t l_hash_hex = dap_hash_sha3_256_to_str_struct(&l_hash_pkey);
            dap_json_object_add_string(a_json_out, a_version == 1 ? "Public key hash" : "pkey_hash", l_hash_hex.s);
        }
    }
