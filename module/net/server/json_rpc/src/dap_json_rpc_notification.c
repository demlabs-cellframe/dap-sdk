...
int dap_json_rpc_notification_registration(const char *a_method, notification_handler_func_t *a_notification_func)
{
    ...
    dap_ht_find_str(s_handler_notifications, a_method, l_handler);
    if (l_handler == 0){
        ...
        dap_ht_add_str(s_handler_notifications, method, l_handler);
...
void dap_json_rpc_notification_unregistration(const char *a_method)
{
    ...
    dap_ht_find_str(s_handler_notifications, a_method, l_handler);
    if (l_handler != NULL){
        dap_ht_del(s_handler_notifications, l_handler);
...
{
    dap_json_rpc_notification_handler_t *l_handler = NULL;
    dap_ht_find_str(s_handler_notifications, a_name_method, l_handler);
    if (l_handler != NULL){
