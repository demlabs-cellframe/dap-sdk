...
bool dap_http_ban_list_client_check(const char *a_addr, dap_hash_t *a_decree_hash, dap_time_t *a_ts) {
...
int dap_http_ban_list_client_add(const char *a_addr, dap_hash_t a_decree_hash, dap_time_t a_ts) {
...
static void s_dap_http_ban_list_client_dump_single(ban_record_t *a_rec, dap_json_t *a_jobj_out) {
    const char *l_decree_hash_str = dap_hash_to_str_static(&a_rec->decree_hash);
...
dap_json_t *dap_http_ban_list_client_dump(const char *a_addr) {
    ...
    if (a_addr) {
        ...
        if (l_rec)
            ...
            dap_json_object_add_string(l_jobj_out, a_addr, "Address is not banlisted");
    } else {
        l_jobj_array = dap_json_array_new();
        if (!l_jobj_array) {
        ...
        HASH_ITER(hh, s_ban_list, l_rec, l_tmp) {
            ...
            dap_json_object_add_int(l_jobj_addr, "num", num++);
            s_dap_http_ban_list_client_dump_single(l_rec, l_jobj_addr);
            dap_json_array_add(l_jobj_array, l_jobj_addr);
        }
    }
    pthread_rwlock_unlock(&s_ban_list_lock);
    return l_jobj_out;
