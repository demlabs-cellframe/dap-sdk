...
generate_wrap_file() {
    ...
    if [ -n "$mock_functions" ]; then
        ...
        if [[ "$OSTYPE" == "darwin"* ]]; then
            ...
            # Strategy:
            # 1. Use -alias to redirect func → __wrap_func
            # 2. Generate assembly trampolines for __real_func access
            
            local trampoline_dir="$(dirname "$wrap_file")"
            local trampoline_s="${trampoline_dir}/mock_trampolines_macos.s"
            local trampoline_o="${trampoline_dir}/mock_trampolines_macos.o"
            
            # Generate assembly file with trampolines
            cat > "$trampoline_s" << 'EOF_HEADER'
# Auto-generated trampolines for macOS ld64 mocking
# This provides __real_ symbols that call original functions
# before -alias redirection takes effect
# Generated by dap_mock_generate.sh
...
EOF_HEADER
            ...
            echo "$mock_functions" | while read func; do
                ...
                cat >> "$trampoline_s" << EOF

# Trampoline for ${func}
.globl ___real_${func}
.weak_definition ___real_${func}
___real_${func}:
    # Jump to original function (before aliasing)
    # We use indirect jump through saved pointer
    leaq _${func}_original(%rip), %rax
    jmpq *(%rax)

# Storage for original function pointer
.section __DATA,__data
.globl _${func}_original
.weak_definition _${func}_original
_${func}_original:
    .quad _${func}  # Address of original function

EOF
            done
            
            # Generate footer
            cat >> "$trampoline_s" << 'EOF_FOOTER'

# End of trampolines
EOF_FOOTER
            
            # Compile trampoline assembly
            if command -v clang &> /dev/null; then
                clang -c -o "$trampoline_o" "$trampoline_s" 2>/dev/null || {
                    print_error "Failed to compile macOS trampolines"
                    return 1
                }
                
                # Add trampoline object to link line
                echo "$trampoline_o" >> "$wrap_file"
                print_success "Generated $func_count -alias options + trampolines (macOS ld64)"
            else
                print_error "clang not found, cannot compile trampolines for macOS"
                return 1
            fi
        else
