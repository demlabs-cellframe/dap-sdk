...
**Note:** All mocks are enabled by default via `DAP_MOCK_CUSTOM`. To disable a specific mock, use `DAP_MOCK_SET_ENABLED(func_name, false)`.
...
void setup_test(void) {
    // Mocks are automatically initialized and enabled by default
    // To disable a specific mock if needed:
