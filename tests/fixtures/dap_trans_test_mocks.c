...
/**
 * @file dap_trans_test_mocks.c
 * @brief Common mock implementations for trans unit tests
 */
...
#define LOG_TAG "dap_trans_test_mocks"
...
)
{
    ...
    // Return mock server if set, otherwise return default mock
    dap_server_t *l_server = dap_trans_test_get_mock_server();
    ...
    dap_http_server_t *l_http_server = dap_trans_test_get_mock_http_server();
    if (l_http_server) {
...
// Mock flow server instance. Keep the real layout because some UDP tests use
// production accessors such as dap_io_flow_server_set_inheritor().
static dap_io_flow_server_t s_mock_io_flow_server = {0};

// External mock state variables (generated by DAP_MOCK_WRAPPER_CUSTOM macros)
extern dap_mock_function_state_t *g_mock_dap_io_flow_server_new;
extern dap_mock_function_state_t *g_mock_dap_io_flow_server_new_datagram;
...
// Wrapper for dap_io_flow_server_new_datagram (was dap_io_flow_server_new_udp)
DAP_MOCK_WRAPPER_CUSTOM(void*, dap_io_flow_server_new_datagram,
    PARAM(const char*, a_name),
    PARAM(void*, a_ops),
    PARAM(void*, a_datagram_ops)
)
{
    UNUSED(a_name);
    UNUSED(a_ops);
    UNUSED(a_datagram_ops);
    
    // Return mock value if set, otherwise return mock instance
    if (g_mock_dap_io_flow_server_new_datagram && g_mock_dap_io_flow_server_new_datagram->return_value.ptr) {
        return g_mock_dap_io_flow_server_new_datagram->return_value.ptr;
    }
    return &s_mock_io_flow_server;
}
