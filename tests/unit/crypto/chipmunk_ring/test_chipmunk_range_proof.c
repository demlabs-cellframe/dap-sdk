...
static void test_uint256_large_amount(void)
{
    ...
    /* Value must be < Q (3168257) for Pedersen commitment to be uniquely
     * determined. Q fits in 22 bits, so we use a value just under Q. */
    uint8_t l_value[32] = {
    ...
    /* l_value as uint64_t LE = 0x301000 = 3145728 < Q=3168257 */
