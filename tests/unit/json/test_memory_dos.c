...
static bool s_test_stack_exhaustion(void) {
    ...
    if (l_json) {
        ...
        // Verify structure is parseable (can't traverse without proper API)
        // For now, just confirm it parsed successfully
        DAP_TEST_FAIL_IF(!l_json, "Nested structure parsed");
    } else {
