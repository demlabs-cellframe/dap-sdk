...
// ============================================================================
// MULTI-CLIENT REGRESSION TESTS
// ============================================================================

/**
 * @brief REGRESSION TEST: Multi-client concurrent writes shouldn't interfere
 * 
 * This test reproduces the multi-client issue observed in integration tests:
 * - Client 1 and Client 2 both send packets
 * - Client 1's flow_ctrl retransmissions should NOT block Client 2
 * - Each client should have independent packet flow
 * 
 * The test validates at UDP trans level (not full integration):
 * 1. Two UDP contexts can exist simultaneously
 * 2. Each context maintains independent state (seq_num, session_id)
 * 3. Packets from one client don't corrupt other client's state
 */
static void test_19_multi_client_isolation(void)
{
    TEST_INFO("Testing multi-client isolation (regression test)");
    
    // ========================================================================
    // STEP 1: Create two independent UDP client contexts
    // ========================================================================
    
    uint64_t l_session_id_1 = 0x1111111111111111;
    uint64_t l_session_id_2 = 0x2222222222222222;
    
    dap_net_trans_udp_ctx_t *l_client1_ctx = dap_udp_test_create_mock_client_ctx(
        l_session_id_1, DAP_ENC_KEY_TYPE_SALSA2012, "127.0.0.1", 8080);
    dap_net_trans_udp_ctx_t *l_client2_ctx = dap_udp_test_create_mock_client_ctx(
        l_session_id_2, DAP_ENC_KEY_TYPE_SALSA2012, "127.0.0.1", 8080);
    
    TEST_ASSERT_NOT_NULL(l_client1_ctx, "Client 1 context should be created");
    TEST_ASSERT_NOT_NULL(l_client2_ctx, "Client 2 context should be created");
    TEST_ASSERT(l_client1_ctx->session_id != l_client2_ctx->session_id,
                "Clients should have different session IDs");
    
    TEST_INFO("✅ Created two independent UDP client contexts");
    TEST_INFO("   Client 1: session=0x%016lX, seq=%u", 
              l_client1_ctx->session_id, l_client1_ctx->seq_num);
    TEST_INFO("   Client 2: session=0x%016lX, seq=%u", 
              l_client2_ctx->session_id, l_client2_ctx->seq_num);
    
    // ========================================================================
    // STEP 2: Setup mock streams for both clients
    // ========================================================================
    ...
    dap_stream_t *l_stream1 = dap_udp_test_setup_mock_stream_with_session(l_trans, l_client1_ctx);
    dap_stream_t *l_stream2 = dap_udp_test_setup_mock_stream_with_session(l_trans, l_client2_ctx);
    
    TEST_ASSERT_NOT_NULL(l_stream1, "Stream 1 should be created");
    TEST_ASSERT_NOT_NULL(l_stream2, "Stream 2 should be created");
    
    TEST_INFO("✅ Created two mock streams");
    
    // ========================================================================
    // STEP 3: Interleaved packet sending (simulates concurrent clients)
    // ========================================================================
    
    #define PACKETS_PER_CLIENT 5
    
    // Track write success for each client
    int l_client1_success = 0;
    int l_client2_success = 0;
    
    for (int i = 0; i < PACKETS_PER_CLIENT; i++) {
        // Client 1 sends
        char l_data1[64];
        snprintf(l_data1, sizeof(l_data1), "Client1 packet %d", i + 1);
        ssize_t l_written1 = l_trans->ops->write(l_stream1, l_data1, strlen(l_data1) + 1);
        if (l_written1 > 0) l_client1_success++;
        
        // Client 2 sends
        char l_data2[64];
        snprintf(l_data2, sizeof(l_data2), "Client2 packet %d", i + 1);
        ssize_t l_written2 = l_trans->ops->write(l_stream2, l_data2, strlen(l_data2) + 1);
        if (l_written2 > 0) l_client2_success++;
    }
    
    TEST_INFO("✅ Sent interleaved packets - Client1: %d, Client2: %d",
              l_client1_success, l_client2_success);
    
    // ========================================================================
    // STEP 4: Validate both clients could send all packets
    // ========================================================================
    
    TEST_ASSERT(l_client1_success == PACKETS_PER_CLIENT,
                "Client 1 should send all %d packets (got %d)", 
                PACKETS_PER_CLIENT, l_client1_success);
    TEST_ASSERT(l_client2_success == PACKETS_PER_CLIENT,
                "Client 2 should send all %d packets (got %d)", 
                PACKETS_PER_CLIENT, l_client2_success);
    
    TEST_INFO("✅ Both clients sent all packets successfully");
    
    // ========================================================================
    // STEP 5: Verify contexts remain independent
    // ========================================================================
    
    // Verify session IDs are still different (no cross-contamination)
    TEST_ASSERT(l_client1_ctx->session_id == l_session_id_1,
                "Client 1 session should remain 0x%lX", l_session_id_1);
    TEST_ASSERT(l_client2_ctx->session_id == l_session_id_2,
                "Client 2 session should remain 0x%lX", l_session_id_2);
    TEST_ASSERT(l_client1_ctx->handshake_key != l_client2_ctx->handshake_key,
                "Clients should have different encryption keys");
    
    TEST_INFO("✅ Context isolation verified - sessions and keys remain independent");
    
    // ========================================================================
    // CLEANUP
    // ========================================================================
    
    dap_udp_test_cleanup_mock_stream(l_stream1);
    dap_udp_test_cleanup_mock_stream(l_stream2);
    dap_udp_test_cleanup_mock_client_ctx(l_client1_ctx);
    dap_udp_test_cleanup_mock_client_ctx(l_client2_ctx);
    DAP_MOCK_DISABLE(dap_events_socket_write_unsafe);
    dap_udp_test_reset_captured_packet();
    
    #undef PACKETS_PER_CLIENT
    
    TEST_SUCCESS("Multi-client isolation test PASSED - clients are independent");
}

/**
 * @brief REGRESSION TEST: Simulated packet loss for one client shouldn't affect others
 * 
 * Simulates the scenario from integration test where:
 * - Client 1 has "lost" ACKs (requires retransmission)
 * - Client 2 should still function normally
 * - Server-side flow routing should keep them isolated
 */
static void test_20_packet_loss_isolation(void)
{
    TEST_INFO("Testing packet loss isolation (multi-client regression)");
    
    // ========================================================================
    // STEP 1: Setup two clients with different states
    // ========================================================================
    
    uint64_t l_session_id_stuck = 0xAAAAAAAAAAAAAAAA;  // "Stuck" client (lost ACKs)
    uint64_t l_session_id_ok = 0xBBBBBBBBBBBBBBBB;    // Normal client
    
    dap_net_trans_udp_ctx_t *l_stuck_ctx = dap_udp_test_create_mock_client_ctx(
        l_session_id_stuck, DAP_ENC_KEY_TYPE_SALSA2012, "127.0.0.1", 8080);
    dap_net_trans_udp_ctx_t *l_ok_ctx = dap_udp_test_create_mock_client_ctx(
        l_session_id_ok, DAP_ENC_KEY_TYPE_SALSA2012, "127.0.0.1", 8080);
    
    TEST_ASSERT_NOT_NULL(l_stuck_ctx, "Stuck client context should be created");
    TEST_ASSERT_NOT_NULL(l_ok_ctx, "OK client context should be created");
    
    TEST_INFO("✅ Created two client contexts:");
    TEST_INFO("   'Stuck' client: session=0x%lX (simulates lost ACKs)", l_session_id_stuck);
    TEST_INFO("   'OK' client:    session=0x%lX (normal operation)", l_session_id_ok);
    
    // ========================================================================
    // STEP 2: Setup streams
    // ========================================================================
    ...
    dap_stream_t *l_stuck_stream = dap_udp_test_setup_mock_stream_with_session(l_trans, l_stuck_ctx);
    dap_stream_t *l_ok_stream = dap_udp_test_setup_mock_stream_with_session(l_trans, l_ok_ctx);
    
    TEST_ASSERT_NOT_NULL(l_stuck_stream, "Stuck stream should be created");
    TEST_ASSERT_NOT_NULL(l_ok_stream, "OK stream should be created");
    
    // ========================================================================
    // STEP 3: "Stuck" client sends many packets (simulating pending retransmits)
    // ========================================================================
    
    TEST_INFO("Step 3: 'Stuck' client sends 20 packets (pending retransmit)...");
    
    uint32_t l_stuck_initial_seq = l_stuck_ctx->seq_num;
    for (int i = 0; i < 20; i++) {
        dap_udp_test_reset_captured_packet();
        char l_data[64];
        snprintf(l_data, sizeof(l_data), "Stuck packet %d", i + 1);
        ssize_t l_written = l_trans->ops->write(l_stuck_stream, l_data, strlen(l_data) + 1);
        TEST_ASSERT(l_written > 0, "Stuck packet %d should be written", i + 1);
    }
    
    uint32_t l_stuck_after_send = l_stuck_ctx->seq_num;
    TEST_INFO("   Stuck client seq: %u -> %u (sent 20 packets)", 
              l_stuck_initial_seq, l_stuck_after_send);
    
    // ========================================================================
    // STEP 4: "OK" client tries to operate while "stuck" client has pending data
    // ========================================================================
    
    TEST_INFO("Step 4: 'OK' client sends 5 packets (should work independently)...");
    
    uint32_t l_ok_initial_seq = l_ok_ctx->seq_num;
    int l_ok_success_count = 0;
    
    for (int i = 0; i < 5; i++) {
        dap_udp_test_reset_captured_packet();
        char l_data[64];
        snprintf(l_data, sizeof(l_data), "OK packet %d", i + 1);
        ssize_t l_written = l_trans->ops->write(l_ok_stream, l_data, strlen(l_data) + 1);
        
        if (l_written > 0) {
            l_ok_success_count++;
            
            // Validate the packet belongs to OK client
            dap_udp_test_captured_packet_t *l_cap = dap_udp_test_get_captured_packet();
            if (l_cap->is_valid) {
                size_t l_buf_size = l_cap->size + 256;
                uint8_t *l_dec = DAP_NEW_SIZE(uint8_t, l_buf_size);
                size_t l_dec_size = dap_enc_decode(l_ok_ctx->handshake_key, 
                                                    l_cap->data, l_cap->size,
                                                    l_dec, l_buf_size, DAP_ENC_DATA_TYPE_RAW);
                if (l_dec_size >= sizeof(dap_stream_trans_udp_full_header_t)) {
                    extern const dap_serialize_schema_t g_udp_full_header_schema;
                    dap_stream_trans_udp_full_header_t l_hdr;
                    dap_deserialize_result_t l_deser = dap_deserialize_from_buffer_raw(
                        &g_udp_full_header_schema, l_dec,
                        sizeof(dap_stream_trans_udp_full_header_t), &l_hdr, NULL);
                    if (l_deser.error_code == 0) {
                        TEST_ASSERT(l_hdr.session_id == l_session_id_ok,
                                    "OK client packet should have OK session ID");
                    }
                }
                DAP_DELETE(l_dec);
            }
    ...
    uint32_t l_ok_after_send = l_ok_ctx->seq_num;
    
    // ========================================================================
    // STEP 5: Validate isolation - OK client should NOT be affected by stuck client
    // ========================================================================
    
    TEST_ASSERT(l_ok_success_count == 5,
                "REGRESSION: OK client should send all 5 packets (got %d)", l_ok_success_count);
    TEST_ASSERT(l_ok_after_send == l_ok_initial_seq + 5,
                "REGRESSION: OK client seq should advance by 5 (got %u -> %u)",
                l_ok_initial_seq, l_ok_after_send);
    
    TEST_INFO("✅ Isolation verified:");
    TEST_INFO("   OK client sent all %d packets successfully", l_ok_success_count);
    TEST_INFO("   OK client seq: %u -> %u", l_ok_initial_seq, l_ok_after_send);
    TEST_INFO("   Stuck client state unchanged (still pending retransmits)");
    
    // ========================================================================
    // STEP 6: Validate stuck client didn't affect OK client's encryption/state
    // ========================================================================
    
    // Verify encryption keys are still independent
    TEST_ASSERT(l_stuck_ctx->handshake_key != l_ok_ctx->handshake_key,
                "Clients should have independent encryption keys");
    TEST_ASSERT(l_stuck_ctx->session_id != l_ok_ctx->session_id,
                "Clients should have independent session IDs");
    
    TEST_INFO("✅ State independence verified: encryption keys and sessions are separate");
    
    // ========================================================================
    // CLEANUP
    // ========================================================================
    
    dap_udp_test_cleanup_mock_stream(l_stuck_stream);
    dap_udp_test_cleanup_mock_stream(l_ok_stream);
    dap_udp_test_cleanup_mock_client_ctx(l_stuck_ctx);
    dap_udp_test_cleanup_mock_client_ctx(l_ok_ctx);
    DAP_MOCK_DISABLE(dap_events_socket_write_unsafe);
    dap_udp_test_reset_captured_packet();
    
    TEST_SUCCESS("Packet loss isolation test PASSED - clients operate independently");
}
...
int main(int argc, char *argv[])
{
    ...
    // Multi-client regression tests
    TEST_RUN(test_19_multi_client_isolation);
    TEST_RUN(test_20_packet_loss_isolation);
