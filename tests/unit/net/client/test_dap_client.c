...
static void test_08_transport_type(void)
{
    ...
    dap_stream_transport_type_t l_new_type = dap_client_get_transport_type(l_client);
    TEST_ASSERT(l_new_type == DAP_STREAM_TRANSPORT_UDP_RELIABLE, 
    ...
    TEST_ASSERT(l_new_type == DAP_STREAM_TRANSPORT_WEBSOCKET, 
    ...
    l_new_type = dap_client_get_transport_type(l_client);
    TEST_ASSERT(l_new_type == DAP_STREAM_TRANSPORT_TLS_DIRECT, 
                "Transport type should be set to TLS_DIRECT");
    
    // Test: Verify field is accessible directly
    TEST_ASSERT(l_client->transport_type == DAP_STREAM_TRANSPORT_TLS_DIRECT, 
