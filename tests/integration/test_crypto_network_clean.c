...
static bool s_mt_node_recovery_callback(void* a_arg) {
    mt_node_context_t* l_node = (mt_node_context_t*)a_arg;
    ...
    if (!l_node->is_online) {
        ...
        if (!l_node->signature_ready) {
            ...
            return s_mt_node_process_consensus(a_arg);
        }
    }
...
static bool s_test_distributed_consensus_workflow(void) {
    ...
    int l_hash_ret = dap_hash_sha3_256(l_proposal_json, strlen(l_proposal_json), &l_proposal_hash);
...
static bool s_test_network_fault_tolerance(void) {
    ...
    dap_hash_sha3_256(l_critical_message, strlen(l_critical_message), &g_current_consensus_hash);
    ...
    if (!g_mt_nodes[1].is_online) {
        dap_proc_thread_timer_add(
            ...
            (dap_thread_timer_callback_t)s_mt_node_recovery_callback,
            &g_mt_nodes[1],
            50000 // 50ms delay
        );
    }
 ...
 */
int main(void) {
    log_it(L_INFO, "Starting Crypto-Network-I/O Integration Tests");
    log_it(L_INFO, "=================================================");
