...
#define LOG_TAG "test_transport_helpers"

/**
 * @brief Create test data for stream testing
 */
uint8_t *test_transport_create_test_data(size_t a_size)
{
 ...
 */
bool test_transport_verify_data(const void *a_sent, const void *a_received, size_t a_size)
{
 ...
 */
int test_stream_ch_context_init(test_stream_ch_context_t *a_ctx, 
                                 char a_channel_id, 
                                 size_t a_data_size)
{
    if (!a_ctx) {
 ...
 */
void test_stream_ch_context_cleanup(test_stream_ch_context_t *a_ctx)
{
    if (!a_ctx) {
        return;
    }
                                      ...
                                      void *a_arg)
{
    ...
    if (!l_ctx) {
        log_it(L_ERROR, "test_stream_ch_receive_callback: context is NULL");
        return;
    }
 ...
 */
bool test_transport_wait_for_streaming(dap_client_t *a_client, uint32_t a_timeout_ms)
{
    if (!a_client) {
        return false;
    }
