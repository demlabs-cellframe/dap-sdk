...
/*
 * Regression: deleting server after stop/wait must not dereference freed listener memory.
 */
...
int main(void)
{
    ...
    for (int i = 0; i < TEST_ITERS; i++) {
        int l_common_rc = dap_common_init("test_server_delete_after_wait_race", NULL);
        if (l_common_rc != 0) {
            fprintf(stderr, "iter=%d dap_common_init failed: %d\n", i, l_common_rc);
            return 1;
        }
