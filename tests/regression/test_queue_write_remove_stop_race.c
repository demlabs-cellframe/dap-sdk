...
static void *s_writer_thread(void *a_arg)
{
    ...
    static const char l_payload[256] = {
        [0 ... 254] = 'R',
        [255] = '\n'
    };
    ...
    while (!atomic_load_explicit(&l_shared->stop, memory_order_acquire)) {
        ...
        (void)dap_events_socket_write(l_worker, l_uuid, l_payload, sizeof(l_payload));
    }
...
int main(void)
{
    ...
    for (int i = 0; i < TEST_ITERS; i++) {
        ...
        if (dap_common_init("test_queue_write_remove_stop_race", NULL) != 0) {
