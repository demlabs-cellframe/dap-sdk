...
int main(void) {
...
#if DAP_PLATFORM_X86
    {
        ...
        /* CRYSTALS reference scalar Montgomery reduce + NTT for verification */
        {
            ...
            for (int i = 0; i < 256; i++) {
                if (crystals_ref[i] != tmp_c[i]) {
                    int64_t diff = (int64_t)crystals_ref[i] - (int64_t)tmp_c[i];
                    ...
                    if (r != 0) {
                        ...
                        if (ntt_modq_fail_cnt <= 4)
                            printf("  CRYSTALS FAIL[%d]: ref=%d fused=%d diff=%lld\n",
                                   i, crystals_ref[i], tmp_c[i], (long long)diff);
                    }
                }
            }
            printf("  Fused vs CRYSTALS ref exact: %s\n", ntt_ok ? "OK" : "FAIL");
            printf("  Fused vs CRYSTALS ref mod-Q: %s (%d failures)\n\n",
                   ntt_modq_ok ? "OK" : "FAIL", ntt_modq_fail_cnt);

            if (!ntt_ok && ntt_modq_ok)
                printf("  (Exact mismatch but mod-Q OK = different bounded representatives)\n\n");
