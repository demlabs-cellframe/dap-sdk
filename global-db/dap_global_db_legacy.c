...
dap_global_db_pkt_old_t *dap_global_db_pkt_pack_old(dap_global_db_pkt_old_t *a_old_pkt, dap_global_db_pkt_old_t *a_new_pkt)
{
    if (!a_new_pkt)
        return a_old_pkt;
    dap_global_db_pkt_old_t *l_old_pkt_new = DAP_REALLOC_RET_VAL_IF_FAIL(a_old_pkt,
        sizeof(dap_global_db_pkt_old_t) + a_old_pkt->data_size + a_new_pkt->data_size, NULL);
