...
int dap_global_db_init()
{
    ...
    if (s_context_global_db == NULL) {
        ...
        l_dbi->storage_path = l_gdb_path_cfg ? dap_strdup(l_gdb_path_cfg) : dap_strdup_printf("%s/var/lib/global_db", g_sys_dir_path);
        const char *l_driver_name = dap_config_get_item_str(g_config, "global_db", "driver");
        l_dbi->driver_name = dap_strdup(l_driver_name ? l_driver_name :
#ifdef DAP_OS_DARWIN
                                                                        "sqlite3");
#else
                                                                        "mdbx");
                             ...
                             const size_t a_value_length, bool a_pin_value, dap_nanotime_t a_timestamp)
{
    ...
    dap_store_obj_t l_store_data = {
        ...
        .flags      = RECORD_DEL_HISTORY_MODIFY | (a_pin_value ? RECORD_PINNED : 0),
        .group      = (char*)a_group,
...
{
    return s_set_unsafe_with_ts(a_global_db_context, a_group, a_key, a_value, a_value_length, a_pin_value, dap_nanotime_now());
}
...
int s_db_object_pin_unsafe(dap_global_db_context_t *a_global_db_context, const char *a_group, const char *a_key, bool a_pin)
{
    ...
    if (l_store_obj) {
        if (a_pin)
            ...
            l_store_obj->flags ^= RECORD_PINNED;
        if ( (l_res = dap_global_db_set_raw_unsafe(a_global_db_context, l_store_obj, 1)) )
        {
...
{
    dap_store_obj_t l_store_obj = {
        .flags      = RECORD_DEL_HISTORY_MODIFY,
        .key        = a_key,
        .group      = (char*)a_group,
