...
static struct sync_obj_data_callback* s_global_db_obj_data_callback_new() {
    ...
    l_callback->hdr.uid = s_global_db_save_callback_data(s_context_global_db, l_callback);
    return l_callback;
}
                                 ...
                                 bool *a_is_pinned, dap_nanotime_t *a_ts)
{
    ...
    if (!l_store_obj) {
        log_it(L_ERROR, "l_store_ibj is not initialized, can't call dap_global_db_get_unsafe");
        return NULL;
    }
    if (a_data_size)
...
byte_t *dap_global_db_get_sync(const char *a_group, const char *a_key, size_t *a_data_size, bool *a_is_pinned, dap_nanotime_t *a_ts)
{
    ...
    if (l_args->hdr.timeout == false) s_global_db_obj_data_callback_destroy(l_args);
    return l_ret;
}
