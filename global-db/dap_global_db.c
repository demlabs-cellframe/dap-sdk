                                                  ...
                                                  const char *a_group, size_t *a_objs_count)
{
    ...
    if (l_store_objs) {
        ...
        for(size_t i = 0; i < l_values_count; i++) {
            l_objs[i] = (dap_global_db_obj_t) {
                    .id         = l_store_objs[i].id,
                    .is_pinned  = l_store_objs[i].flags & RECORD_PINNED,
                    .key = dap_strdup(l_store_objs[i].key),
                    .value = DAP_DUP_SIZE(l_store_objs[i].value, l_store_objs[i].value_len),
                    .value_len = l_store_objs[i].value_len,
                    .timestamp = l_store_objs[i].timestamp
            };
        }
    }
