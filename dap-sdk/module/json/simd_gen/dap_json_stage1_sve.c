...
/**
 * @file dap_json_stage1_sve.c
 * @brief SimdJSON-style Stage 1 tokenization with SVE SIMD optimization
...
#define LOG_TAG "dap_json_stage1_sve"
...
// ============================================================================
#include "dap_json_stage1_sve_arch.h"
...
static dap_json_bitmaps_sve_t s_classify_chunk_sve(const uint8_t *a_chunk)
{
    dap_json_bitmaps_sve_t bitmaps = {0};
    ...
    bitmaps.whitespace = (MASK_TYPE)dap_sve_movemask_u8(whitespace);
    bitmaps.quote = (MASK_TYPE)dap_sve_movemask_u8(quote);
    bitmaps.backslash = (MASK_TYPE)dap_sve_movemask_u8(backslash);
    bitmaps.structural = (MASK_TYPE)dap_sve_movemask_u8(structural);
...
static size_t s_scan_string_fast_sve(
...
)
{
    ...
    while (l_pos + chunk_size <= l_len) {
        ...
        MASK_TYPE quote_mask = (MASK_TYPE)dap_sve_movemask_u8(has_quote);
        MASK_TYPE backslash_mask = (MASK_TYPE)dap_sve_movemask_u8(has_backslash);
 ...
 */
static inline int s_add_token_sve(
...
/* ========================================================================== */
/*                    MAIN TOKENIZATION - SVE                       */
/* ========================================================================== */

/**
 * @brief Full SIMD-optimized Stage 1 tokenization (SVE)
...
int dap_json_stage1_run_sve(dap_json_stage1_t *a_stage1)
{
    ...
    debug_if(dap_json_get_debug(), L_DEBUG, "Starting SVE SimdJSON Stage 1 tokenization (%zu bytes)", input_len);
    ...
    while (pos < chunk_end) {
        ...
        dap_json_bitmaps_sve_t bitmaps = s_classify_chunk_sve(input + pos);
        ...
        while (chunk_pos < chunk_limit) {
            ...
            if (bit_offset < chunk_size && (bitmaps.structural & (((MASK_TYPE)1) << bit_offset))) {
                int ret = s_add_token_sve(a_stage1, (uint32_t)chunk_pos, 0, TOKEN_TYPE_STRUCTURAL, c);
            ...
            if (c == '"') {
                size_t end = s_scan_string_fast_sve(a_stage1, chunk_pos);
                if (end == chunk_pos) return a_stage1->error_code;
                
                size_t str_len = end - chunk_pos;
                int ret = s_add_token_sve(a_stage1, (uint32_t)chunk_pos, (uint32_t)str_len,
                                     TOKEN_TYPE_STRING, 0);
                if (ret != STAGE1_SUCCESS) return ret;
            ...
            if (c == '-' || (c >= '0' && c <= '9')) {
                ...
                int ret = s_add_token_sve(a_stage1, (uint32_t)chunk_pos, (uint32_t)num_len,
                                     TOKEN_TYPE_NUMBER, 0);
                if (ret != STAGE1_SUCCESS) return ret;
            ...
            if (c == 't' || c == 'f' || c == 'n') {
                ...
                int ret = s_add_token_sve(a_stage1, (uint32_t)chunk_pos, (uint32_t)lit_len,
    ...
    while (pos < input_len) {
        ...
        // Structural
        if (c == '{' || c == '}' || c == '[' || c == ']' || c == ':' || c == ',') {
            int ret = s_add_token_sve(a_stage1, (uint32_t)pos, 0, TOKEN_TYPE_STRUCTURAL, c);
            if (ret != STAGE1_SUCCESS) return ret;
            a_stage1->structural_chars++;
            pos++;
        }
        // String
            ...
            size_t end = s_scan_string_fast_sve(a_stage1, pos);
            if (end == pos) return a_stage1->error_code;
            
            size_t str_len = end - pos;
            int ret = s_add_token_sve(a_stage1, (uint32_t)pos, (uint32_t)str_len,
                                 TOKEN_TYPE_STRING, 0);
            if (ret != STAGE1_SUCCESS) return ret;
        ...
        // Number
            ...
            int ret = s_add_token_sve(a_stage1, (uint32_t)pos, (uint32_t)num_len,
                                 TOKEN_TYPE_NUMBER, 0);
            if (ret != STAGE1_SUCCESS) return ret;
        ...
        // Literal
            ...
            int ret = s_add_token_sve(a_stage1, (uint32_t)pos, (uint32_t)lit_len,
                                 TOKEN_TYPE_LITERAL, lit_type);
            if (ret != STAGE1_SUCCESS) return ret;
    ...
    debug_if(dap_json_get_debug(), L_DEBUG, "SVE SimdJSON Stage 1 complete: %zu tokens (%zu structural, %zu strings, %zu numbers, %zu literals)",
 ...
 */
void dap_json_stage1_sve_set_debug(bool a_enable)
{
    // SVE now uses global dap_json_get_debug(), no local flag needed
    (void)a_enable;
}
